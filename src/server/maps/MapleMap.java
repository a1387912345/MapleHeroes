...
import static server.MapleStatInfo.damage;
...
public final class MapleMap {
    ...
    */
    ...
    private String mapName, streetName, onUserEnter, onFirstUserEnter, speedRunLeader = "", fieldScript;
    ...
    public final void MapleClient(MapleClient c) {
    this.c = c;
}
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
        ...
        if (!instanced && mapobjects.get(MapleMapObjectType.ITEM).size() >= 255) {
            removeDrops();
            broadcastMessage(CField.getGameMessage((short) 7, "The drops on the map were cleared in order to reduce lags."));
        }
     ...
     //   final int caServerrate = ChannelServer.getInstance(channel).getCashRate();
        ...
        for (final MonsterGlobalDropEntry de : globalEntry) {
            if (Randomizer.nextInt(999999) < de.chance && (de.continent < 0 || (de.continent < 10 && mapid / 100000000 == de.continent) || (de.continent < 100 && mapid / 10000000 == de.continent) || (de.continent < 1000 && mapid / 1000000 == de.continent))) {
                ...
                if (de.itemId == 0) {
               final MapleMonsterStats mons = mob.getStats();
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {
        ...
        if (monster.getId() == 9400902) {
            ...
            chr.getMap().broadcastMessage(CField.musicChange("Bgm02.img/MissingYou"));
        }
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
        ...
        //final int caServerrate = ChannelServer.getInstance(channel).getCashRate();
        final int cashz = (int) ((monster.getStats().isBoss() && monster.getStats().getHPDisplayType() == 0 ? 3 : 1));
    ...
    public final int spawnMonsterWithEffectBelow(final MapleMonster mob, final Point pos, final int effect) {
        ...
        MapTimer.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                broadcastMessage(CField.removeMist(clock.getObjectId()));
    ...
    public final void spawnZakum(final int x, final int y) {
        ...
        final Point pos = new Point(x, y);
        final MapleMonster mainb = MapleLifeFactory.getMonster(9400900);
        final Point spos = calcPointBelow(new Point(pos.x, pos.y - 1));
        mainb.setPosition(spos);
        mainb.setFake(true);
   ...
   /* 
    ...
    public final void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite, final MapleCharacter chr) {      
    //  MapleClient c;
   //   MapleCharacter chr = (MapleCharacter);
        ...
        if (GameConstants.isChangeable(monster.getId())) {
            //this.getStats().setHp(damage + 1)
          //  final short chr = MapleCharacter.getLevel();
            OverrideMonsterStats newstat = new OverrideMonsterStats(600000, monster.getMp(), monster.getMobExp());
            monster.getStats().setExp(4150);
            monster.getStats().setLevel((short) 110);
            monster.getStats().setSpeed(100);
            monster.setOverrideStats(newstat);
        } 
        monster.setMap(this);
        checkRemoveAfter(monster);
    ...
    public final void spawnFamiliar(final MonsterFamiliar familiar, final boolean respawn) {
        spawnAndAddRangedMapObject(familiar, new DelayedPacketCreation() {
            @Override
            public void sendPackets(MapleClient c) {
                if (familiar != null && c.getCharacter() != null) {
                    c.sendPacket(CField.spawnFamiliar(familiar, true, respawn));
                }
            }
        });
    }
    ...
    public final void spawnMobDrop(final Item idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr, final byte droptype, final int questid) {
        ...
        spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            @Override
            public void sendPackets(MapleClient c) {
                if (c != null && c.getCharacter() != null && (questid <= 0 || c.getCharacter().getQuestStatus(questid) == 1) && (idrop.getItemId() / 10000 != 238 || c.getCharacter().getMonsterBook().getLevelByCard(idrop.getItemId()) >= 2) && mob != null && dropPos != null) {
...
//                }
        ...
        if (!chr.isInBlockedMap() && chr.getLevel() > 10 && isTown() && chr.getMapId() % 1000000 == 0 && chr.getMapId() <= 600000000) {
            for (QuickMove qm : QuickMove.values()) {
                if (qm.getMap() == chr.getMapId()) {
                    ...
                    chr.getClient().sendPacket(CField.getQuickMoveInfo(true, qmn));
                    break;
                }
            }
        } else {
            chr.getClient().sendPacket(CField.getQuickMoveInfo(false, new LinkedList()));
        }
        /*for (int npc : GameConstants.unusedNpcs) {
         if (getNPCById(npc) != null && getId() != 910000000) {
         chr.getClient().sendPacket(CField.NPCPacket.toggleNPCShow(getNPCById(npc).getObjectId(), true));
         //hide unused npcs
         }
         }*/
        if (getNPCById(9073000) != null && getId() == 931050410) {
            chr.getClient().sendPacket(CField.NPCPacket.toggleNPCShow(getNPCById(9073000).getObjectId(), true, true));
        }
        if (GameConstants.isPhantom(chr.getJob())) {
            chr.getClient().sendPacket(PhantomPacket.updateCardStack(chr.getCardStack()));
        }
        if (!chr.isClone()) {
        ...
        if (this.mapid == 103050900) {
            try {
                chr.getClient().sendPacket(CField.UIPacket.IntroEnableUI(1));
                Thread.sleep(2000L);
                chr.getClient().sendPacket(CField.UIPacket.getDirectionInfo(1, 8000));
                chr.getClient().sendPacket(CField.UIPacket.getDirectionInfo(1, 8000));
                chr.getClient().sendPacket(CField.UIPacket.getDirectionInfo(3, 2));
            ...
            chr.getClient().sendPacket(CField.UIPacket.getDirectionStatus(false));
            chr.getClient().sendPacket(CField.UIPacket.IntroEnableUI(0));
            chr.getClient().removeClickedNPC();
    ...
    public final void disconnectAll() {
        for (MapleCharacter chr : getCharactersThreadsafe()) {
            if (!chr.isGM()) {
                ...
                chr.getClient().getSocketChannel().close();
            }
        }
    }
        ...
        EtcTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (c.getCharacter() != null) {
                    c.getCharacter().changeMap(MapleMap.this, getPortal(0));
