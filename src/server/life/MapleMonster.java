...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private long hp, nextKill = 0, lastDropTime = 0, finalmaxhp;
    private int mp, exp;
            ...
            default: {
                for (final int i : toSpawn) {
                    ...
                    if (mob.getId() == 9300216) {
                        map.broadcastMessage(CField.environmentChange("Dojang/clear", 5));//was4
                        map.broadcastMessage(CField.environmentChange("dojang/end/clear", 12));//was3
                    }
                }
                break;
            }
    ...
    public final void changeableMob(MapleCharacter chr) {
        if (getStats().isChangeableMob()) {
            ...
            getStats().setExp(chr.getLevel() * 3000);
            setStatChanged(true);
        /*                            if (monster.getId() == 9306300) {
            //this.getStats().setHp(damage + 1);
           // OverrideMonsterStats newstat = new OverrideMonsterStats(damage, this.getMp(), this.getMobExp());
            OverrideMonsterStats oms = new OverrideMonsterStats();
            monster.getStats().setHp(10000000000L);
            monster.getStats().setLevel(from.getLevel());
            monster.getStats().setExp(from.getLevel() * 2000);*/
         //   setOverrideStats(true);
       // }
        }
    }
    ...
    public final void sendSpawnData(final MapleClient client) {
        ...
        if (GameConstants.isAzwanMap(client.getCharacter().getMapId())) {
        ...
        sendStatus(client);
        if (map != null && !stats.isEscort() && client.getCharacter() != null && client.getCharacter().getTruePosition().distanceSq(getTruePosition()) <= GameConstants.maxViewRangeSq_Half()) {
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if (stat == MonsterStatus.POISON || stat == MonsterStatus.VENOMOUS_WEAPON) {
            ...
            if (count >= eff.getDOTStack() && eff.getSourceId() != 4100011 && eff.getSourceId() != 4120018) {
                System.out.println("[Apply Status] return 4");
                return;
            }
        }
        ...
        if (stat == MonsterStatus.POISON || stat == MonsterStatus.VENOMOUS_WEAPON) {
            ...
            if (con != null) {
                ...
                con.getClient().sendPacket(MobPacket.applyMonsterStatus(this, status, con));
            } else {
