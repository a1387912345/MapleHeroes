...
public class GMCommand {
    ...
    public static class Reports extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            for (int x = cheaters.size() - 1; x >= 0; x--) {
                ...
                c.getCharacter().dropMessage(6, cheater.getInfo());
            }
            return 1;
        }
    }
    ...
    public static class ClearReport extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.getCharacter().dropMessage(5, "Done.");
            return 1;
        }
    }
    ...
    public static class ClearDrops extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getCharacter().getMap().removeDrops();
            return 1;
        }
    }
    ...
    public static class Job extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (!MapleJob.isExist(jobid)) {
                c.getCharacter().dropMessage(5, "Invalid Job");
                return 0;
            }
            c.getCharacter().changeJob((short) jobid);
            c.getCharacter().setSubcategory(c.getCharacter().getSubcategory());
            return 1;
        }
    }
    ...
    public static class KillNear extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            MapleMap map = c.getCharacter().getMap();
            List<MapleMapObject> players = map.getMapObjectsInRange(c.getCharacter().getPosition(), (double) 25000, Arrays.asList(MapleMapObjectType.PLAYER));
            for (MapleMapObject closeplayers : players) {
                MapleCharacter playernear = (MapleCharacter) closeplayers;
                if (playernear.isAlive() && playernear != c.getCharacter() && playernear.getJob() != 910) {
                    playernear.setHp(0);
    ...
    public static class ManualEvent extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            if (c.getChannelServer().manualEvent(c.getCharacter())) {
    ...
    public static class WarpMap extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            try {
                ...
                if (target == null) {
                    c.getCharacter().dropMessage(6, "Map does not exist");
                    return 0;
                }
                final MapleMap from = c.getCharacter().getMap();
                for (MapleCharacter chr : from.getCharactersThreadsafe()) {
                    chr.changeMap(target, target.getPortal(0));
                }
            } catch (NumberFormatException e) {
                c.getCharacter().dropMessage(5, "Error: " + e.getMessage());
    ...
    public static class KillAll extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            MapleMonster mob;
            for (MapleMapObject monstermo : map.getMapObjectsInRange(c.getCharacter().getPosition(), range, Arrays.asList(MapleMapObjectType.MONSTER))) {
                mob = (MapleMonster) monstermo;
                if (!mob.getStats().isBoss() || mob.getStats().isPartyBonus() || c.getCharacter().isGM()) {
                    map.killMonster(mob, c.getCharacter(), false, false, (byte) 1);
                }
            }
            return 1;
        }
    ...
    public static class KillAllDrops extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            for (MapleMapObject monstermo : map.getMapObjectsInRange(c.getCharacter().getPosition(), range, Arrays.asList(MapleMapObjectType.MONSTER))) {
                mob = (MapleMonster) monstermo;
                if (!mob.getStats().isBoss() || mob.getStats().isPartyBonus() || c.getCharacter().isGM()) {
                    map.killMonster(mob, c.getCharacter(), true, false, (byte) 1);
                }
            }
            return 1;
        }
    ...
    public static class WarpHere extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (victim != null) {
                if (c.getCharacter().inPVP() || (!c.getCharacter().isGM() && (victim.isInBlockedMap() || victim.isGM()))) {
                    c.getCharacter().dropMessage(5, "Try again later.");
                    return 0;
                }
                victim.changeMap(c.getCharacter().getMap(), c.getCharacter().getMap().findClosestPortal(c.getCharacter().getTruePosition()));
            } else {
                int ch = World.Find.findChannel(splitted[1]);
                if (ch < 0) {
                    c.getCharacter().dropMessage(5, "Not found.");
                    return 0;
                }
                victim = ChannelServer.getInstance(ch).getPlayerStorage().getCharacterByName(splitted[1]);
                if (victim == null || victim.inPVP() || (!c.getCharacter().isGM() && (victim.isInBlockedMap() || victim.isGM()))) {
                    c.getCharacter().dropMessage(5, "Try again later.");
                    return 0;
                }
                c.getCharacter().dropMessage(5, "Victim is cross changing channel.");
                victim.dropMessage(5, "Cross changing channel.");
                if (victim.getMapId() != c.getCharacter().getMapId()) {
                    final MapleMap mapp = victim.getClient().getChannelServer().getMapFactory().getMap(c.getCharacter().getMapId());
                    victim.changeMap(mapp, mapp.findClosestPortal(c.getCharacter().getTruePosition()));
                }
    ...
    public static class Position extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            Point position = c.getCharacter().getPosition();

            c.getCharacter().dropMessage(6, "Your position is: " + position + ".");
            return 1;
        }
    }
    ...
    public static class Warp extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            MapleCharacter victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            if (victim != null && c.getCharacter().getGMLevel() >= victim.getGMLevel() && !victim.inPVP() && !c.getCharacter().inPVP()) {
                if (splitted.length == 2) {
                    c.getCharacter().changeMap(victim.getMap(), victim.getMap().findClosestSpawnpoint(victim.getTruePosition()));
                } else {
                    ...
                    if (splitted.length > 3) {
                        try {
                            ...
                            c.getCharacter().dropMessage(5, "Invalid portal selected.");
                ...
                try {
                    ...
                    if (ch < 0) {
                        ...
                        c.getCharacter().changeMap(target, targetPortal);
                    } else {
                        victim = ChannelServer.getInstance(ch).getPlayerStorage().getCharacterByName(splitted[1]);
                        c.getCharacter().dropMessage(6, "Cross changing channel. Please wait.");
                        if (victim.getMapId() != c.getCharacter().getMapId()) {
                            final MapleMap mapp = c.getChannelServer().getMapFactory().getMap(victim.getMapId());
                            c.getCharacter().changeMap(mapp, mapp.findClosestPortal(victim.getTruePosition()));
                        }
                        c.getCharacter().changeChannel(ch);
                    }
                } catch (NumberFormatException e) {
                    c.getCharacter().dropMessage(6, "Something went wrong " + e.getMessage());
                    return 0;
                }
            }
            return 1;
    ...
    public static class TempBan extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (victim == null || reason < 0 || reason >= types.length) {
                c.getCharacter().dropMessage(6, "Unable to find character or reason was not valid, type tempban to see reasons");
                return 0;
            }
            victim.tempban("Temp banned by " + c.getCharacter().getName() + " for " + types[reason] + " reason", cal, reason, ipBan);
            c.getCharacter().dropMessage(6, "The character " + splitted[1] + " has been successfully tempbanned till " + df.format(cal.getTime()));
            return 1;
        }
    }
    ...
    public static class Ban extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getCharacter().dropMessage(5, "[Syntax] !" + getCommand() + " <IGN> <Reason>");
                c.getCharacter().dropMessage(5, "If you want to consider this ban as an autoban, set the reason \"AutoBan\"");
                return 0;
            }
            ...
            if (target != null) {
                if ((c.getCharacter().getGMLevel() > target.getGMLevel() || c.getCharacter().isAdmin()) && !target.getClient().isGm() && !target.isAdmin()) {
                    //sb.append(" (IP: ").append(target.getClient().getSessionIPAddress()).append(")");
                    if (target.ban(sb.toString(), hellban || ipBan, false, hellban)) {
                        c.getCharacter().dropMessage(6, "[" + getCommand() + "] Successfully banned " + splitted[1] + ".");
                        return 1;
                    } else {
                        c.getCharacter().dropMessage(6, "[" + getCommand() + "] Failed to ban.");
                        return 0;
                    }
                } else {
                    c.getCharacter().dropMessage(6, "[" + getCommand() + "] May not ban GMs...");
                    return 1;
                }
            } else {
                if (MapleCharacter.ban(splitted[1], sb.toString(), false, c.getCharacter().isAdmin() ? 250 : c.getCharacter().getGMLevel(), hellban)) {
                    c.getCharacter().dropMessage(6, "[" + getCommand() + "] Successfully offline banned " + splitted[1] + ".");
                    return 1;
                } else {
                    c.getCharacter().dropMessage(6, "[" + getCommand() + "] Failed to ban " + splitted[1]);
                    return 0;
                }
            }
        }
    ...
    public static class Kill extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (splitted.length < 2) {
                c.getCharacter().dropMessage(6, "Syntax: !kill <list player names>");
                return 0;
            }
            ...
            for (int i = 1; i < splitted.length; i++) {
                try {
                    ...
                    c.getCharacter().dropMessage(6, "Player " + splitted[i] + " not found.");
                }
    ...
    public static class Fly extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            SkillFactory.getSkill(1146).getEffect(1).applyTo(c.getCharacter());
            SkillFactory.getSkill(1142).getEffect(1).applyTo(c.getCharacter());
            c.getCharacter().dispelBuff(1146);
            return 1;
        }
    }
    ...
    public static class GivePet extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 6) {
                c.getCharacter().dropMessage(0, splitted[0] + " <character name> <petid> <petname> <petlevel> <petcloseness> <petfullness>");
                return 0;
            }
            ...
            if (id >= 5001000 || id < 5000000) {
                c.getCharacter().dropMessage(0, "Invalid pet id.");
                return 0;
            }
            if (level > 30) {
                level = 30;
    ...
    public static class GetSkill extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.getCharacter().changeSingleSkillLevel(skill, level, masterlevel);
            return 1;
        }
    }
    ...
    public static class Fame extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            MapleCharacter player = c.getCharacter();
            if (splitted.length < 2) {
                c.getCharacter().dropMessage(6, "Syntax: !fame <player> <amount>");
                return 0;
            }
            MapleCharacter victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            int fame;
            try {
                ...
                c.getCharacter().dropMessage(6, "Invalid Number...");
                return 0;
            }
    ...
    public static class SP extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getCharacter().setRemainingSp(CommandProcessorUtil.getOptionalIntArg(splitted, 1, 1));
            c.getCharacter().updateSingleStat(MapleStat.AVAILABLESP, 0);
            return 1;
        }
    }
    ...
    public static class SP2 extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getCharacter().setRemainingSp(CommandProcessorUtil.getOptionalIntArg(splitted, 2, 1), Integer.parseInt(splitted[1]));
            c.getCharacter().updateSingleStat(MapleStat.AVAILABLESP, 0);
            return 1;
        }
    }
    ...
    public static class KillMap extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            for (MapleCharacter map : c.getCharacter().getMap().getCharactersThreadsafe()) {
    ...
    public static class LevelUpTill extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            while (c.getCharacter().getLevel() < Integer.parseInt(splitted[1])) {
                if (c.getCharacter().getLevel() < 255) {
                    c.getCharacter().levelUp();
                }
            }
            //}
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (!c.getCharacter().isAdmin()) {
                for (int i : GameConstants.itemBlock) {
                    if (itemId == i) {
                        c.getCharacter().dropMessage(5, "Sorry but this item is blocked for your GM level.");
                        return 0;
                    }
                }
            }
            ...
            if (!ii.itemExists(itemId)) {
                ...
                if (!c.getCharacter().isAdmin()) {
                    ...
                    item.setGMLog(c.getCharacter().getName() + " used !getitem");
                }
    ...
    public static class PotentialItem extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            if (itemId >= 2000000) {
                c.getCharacter().dropMessage(5, "You can only get equips.");
                ...
                equip.setOwner(c.getCharacter().getName());
    ...
    public static class ProItem extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (splitted.length < 3) {
                c.getCharacter().dropMessage(5, "!proitem id stats potential stats");
                return 0;
            }
            final int itemId = Integer.parseInt(splitted[1]);
            MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
    ...
    public static class Level extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getCharacter().setLevel(Short.parseShort(splitted[1]));
            c.getCharacter().updateSingleStat(MapleStat.LEVEL, Integer.parseInt(splitted[1]));
    ...
    public static class SetEvent extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            MapleEvent.onStartEvent(c.getCharacter());
            return 1;
        }
    }
    ...
    public static class StartEvent extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            if (c.getChannelServer().getEvent() == c.getCharacter().getMapId()) {
                MapleEvent.setEvent(c.getChannelServer(), false);
                c.getCharacter().dropMessage(5, "Started the event and closed off");
                return 1;
            } else {
                c.getCharacter().dropMessage(5, "!event must've been done first, and you must be in the event map.");
                return 0;
            }
        }
    }
    ...
    public static class Event extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (type == null) {
                ...
                c.getCharacter().dropMessage(5, sb.toString().substring(0, sb.toString().length() - 1));
                return 0;
            }
    ...
    public static class RemoveItem extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getCharacter().dropMessage(6, "Need <name> <itemid>");
                return 0;
            }
            MapleCharacter chr = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            if (chr == null) {
                c.getCharacter().dropMessage(6, "This player does not exist");
                return 0;
            }
            chr.removeAll(Integer.parseInt(splitted[2]), false);
            c.getCharacter().dropMessage(6, "All items with the ID " + splitted[2] + " has been removed from the inventory of " + splitted[1] + ".");
            return 1;
    ...
    public static class Smega extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            World.Broadcast.broadcastSmega(CWvsContext.broadcastMsg(3, c.getCharacter() == null ? c.getChannel() : c.getCharacter().getClient().getChannel(), c.getCharacter() == null ? c.getCharacter().getName() : c.getCharacter().getName() + " : " + StringUtil.joinStringFrom(splitted, 1), true));
            /*if (splitted.length < 2) {
    ...
    public static class SpeakMega extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            MapleCharacter victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            if (victim == null) {
                c.getCharacter().dropMessage(0, "The person isn't login, or doesn't exists.");
                return 0;
            }
    ...
    public static class gmtext extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (splitted[1].equalsIgnoreCase("normal")) {
                ...
                c.getCharacter().dropMessage(6, "Wrong syntax: use !gmtext normal/orange/pink/purple/green/blue/red/whitebg/lightinggreen/yellow/spouse");
                return 1;
            }
            try {
                Connection con = DatabaseConnection.getConnection();
                PreparedStatement ps = con.prepareStatement("UPDATE characters SET chatcolour = ? WHERE name = ?");
                ps.setString(2, c.getCharacter().getName());
                ps.setInt(1, text);
                ps.executeUpdate();
                ps.close();
                c.getCharacter().setChatType(text);
    ...
    public static class DiseaseMap extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getCharacter().dropMessage(6, "!disease <type> <level> where type = SEAL/DARKNESS/WEAKEN/STUN/CURSE/POISON/SLOW/SEDUCE/REVERSE/ZOMBIFY/POTION/SHADOW/BLIND/FREEZE/POTENTIAL");
                return 0;
            }
            int type;
            if (splitted[1].equalsIgnoreCase("SEAL")) {
                ...
                c.getCharacter().dropMessage(6, "!disease <type> <level> where type = SEAL/DARKNESS/WEAKEN/STUN/CURSE/POISON/SLOW/SEDUCE/REVERSE/ZOMBIFY/POTION/SHADOW/BLIND/FREEZE/POTENTIAL/SLOW2/TORNADO/FLAG");
                return 0;
            }
            for (MapleCharacter mch : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
                if (mch.getMapId() == c.getCharacter().getMapId()) {
                    if (splitted.length == 4) {
                        if (mch == null) {
                            c.getCharacter().dropMessage(5, "Not found.");
                            return 0;
    ...
    public static class Disease extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getCharacter().dropMessage(6, "!disease <type> [charname] <level> where type = SEAL/DARKNESS/WEAKEN/STUN/CURSE/POISON/SLOW/SEDUCE/REVERSE/ZOMBIFY/POTION/SHADOW/BLIND/FREEZE/POTENTIAL");
                return 0;
            }
            int type;
            if (splitted[1].equalsIgnoreCase("SEAL")) {
                ...
                c.getCharacter().dropMessage(6, "!disease <type> [charname] <level> where type = SEAL/DARKNESS/WEAKEN/STUN/CURSE/POISON/SLOW/SEDUCE/REVERSE/ZOMBIFY/POTION/SHADOW/BLIND/FREEZE/POTENTIAL/SLOW2/TORNADO/FLAG");
                return 0;
            }
            if (splitted.length == 4) {
    ...
    public static class DisposeClones extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getCharacter().dropMessage(6, c.getCharacter().getCloneSize() + " clones disposed.");
            c.getCharacter().disposeClones();
            return 1;
        }
    }
    ...
    public static class SetInstanceProperty extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (em == null || em.getInstances().size() <= 0) {
                c.getCharacter().dropMessage(5, "none");
            } else {
    ...
    public static class ListInstanceProperty extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (em == null || em.getInstances().size() <= 0) {
                ...
                for (EventInstanceManager eim : em.getInstances()) {
                    c.getCharacter().dropMessage(5, "Event " + eim.getName() + ", eventManager: " + em.getName() + " iprops: " + eim.getProperty(splitted[2]) + ", eprops: " + em.getProperty(splitted[2]));
                }
            }
            return 0;
        }
    ...
    public static class LeaveInstance extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (c.getCharacter().getEventInstance() == null) {
                c.getCharacter().dropMessage(5, "You are not in one");
            } else {
                c.getCharacter().getEventInstance().unregisterPlayer(c.getCharacter());
            }
            return 1;
        }
    }
    ...
    public static class WhosThere extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            StringBuilder builder = new StringBuilder("Players on Map: ").append(c.getCharacter().getMap().getCharactersThreadsafe().size()).append(", ");
            for (MapleCharacter chr : c.getCharacter().getMap().getCharactersThreadsafe()) {
                if (builder.length() > 150) { // wild guess :o
                    builder.setLength(builder.length() - 2);
                    c.getCharacter().dropMessage(6, builder.toString());
    ...
    public static class StartInstance extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (c.getCharacter().getEventInstance() != null) {
                c.getCharacter().dropMessage(5, "You are in one");
            } else if (splitted.length > 2) {
                EventManager em = c.getChannelServer().getEventSM().getEventManager(splitted[1]);
                if (em == null || em.getInstance(splitted[2]) == null) {
                    c.getCharacter().dropMessage(5, "Not exist");
                } else {
                    em.getInstance(splitted[2]).registerPlayer(c.getCharacter());
                }
            } else {
                c.getCharacter().dropMessage(5, "!startinstance [eventmanager] [eventinstance]");
            }
            return 1;
    ...
    public static class ResetMobs extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getCharacter().getMap().killAllMonsters(false);
            return 1;
        }
    }
    ...
    public static class KillMonsterByOID extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (monster != null) {
                map.killMonster(monster, c.getCharacter(), false, false, (byte) 1);
            }
            return 1;
        }
    }
    ...
    public static class RemoveNPCs extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getCharacter().getMap().resetNPCs();
            return 1;
        }
    }
    ...
    public static class Yellow extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            byte[] packet = CWvsContext.yellowChat((splitted[0].equals("!y") ? ("[" + c.getCharacter().getName() + "] ") : "") + StringUtil.joinStringFrom(splitted, 2));
            if (range == 0) {
                c.getCharacter().getMap().broadcastMessage(packet);
            } else if (range == 1) {
                ChannelServer.getInstance(c.getChannel()).broadcastPacket(packet);
    ...
    public static class TDrops extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getCharacter().getMap().toggleDrops();
            return 1;
        }
    }
    ...
    public static class Jail extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getCharacter().dropMessage(6, "jail [name] [minutes, 0 = forever]");
                return 0;
            }
            MapleCharacter victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            ...
            if (victim != null && c.getCharacter().getGMLevel() >= victim.getGMLevel()) {
                ...
                c.getCharacter().dropMessage(6, "Please be on their channel.");
                return 0;
            }
            return 1;
        }
    ...
    public static class LookNPC extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            for (MapleMapObject reactor1l : c.getCharacter().getMap().getAllNPCsThreadsafe()) {
                MapleNPC reactor2l = (MapleNPC) reactor1l;
                c.getCharacter().dropMessage(5, "NPC: oID: " + reactor2l.getObjectId() + " npcID: " + reactor2l.getId() + " Position: " + reactor2l.getPosition().toString() + " Name: " + reactor2l.getName());
            }
            return 0;
        }
    }
    ...
    public static class LookReactor extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            for (MapleMapObject reactor1l : c.getCharacter().getMap().getAllReactorsThreadsafe()) {
                MapleReactor reactor2l = (MapleReactor) reactor1l;
                c.getCharacter().dropMessage(5, "Reactor: oID: " + reactor2l.getObjectId() + " reactorID: " + reactor2l.getReactorId() + " Position: " + reactor2l.getPosition().toString() + " State: " + reactor2l.getState() + " Name: " + reactor2l.getName());
            }
            return 0;
        }
    }
    ...
    public static class LookPortals extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            for (MaplePortal portal : c.getCharacter().getMap().getPortals()) {
                c.getCharacter().dropMessage(5, "Portal: ID: " + portal.getId() + " script: " + portal.getScriptName() + " name: " + portal.getName() + " pos: " + portal.getPosition().x + "," + portal.getPosition().y + " target: " + portal.getTargetMapId() + " / " + portal.getTarget());
            }
            return 0;
        }
    }
    ...
    public static class MyNPCPos extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            Point pos = c.getCharacter().getPosition();
            c.getCharacter().dropMessage(6, "X: " + pos.x + " | Y: " + pos.y + " | RX0: " + (pos.x + 50) + " | RX1: " + (pos.x - 50) + " | FH: " + c.getCharacter().getFH());
            return 1;
        }
    }
    ...
    public static class Letter extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getCharacter().dropMessage(6, "syntax: !letter <color (green/red)> <word>");
                return 0;
            }
            int start;
            int nstart;
            if (splitted[1].equalsIgnoreCase("green")) {
                ...
                c.getCharacter().dropMessage(6, "Unknown color!");
                return 0;
            }
            ...
            int w = 32;
            int dStart = c.getCharacter().getPosition().x - (splitString.length() / 2 * 32);
            for (Integer i : chars) {
                if (i.intValue() == -1) {
                    dStart += 32;
                } else {
                    ...
                    if (i.intValue() < 200) {
                        ...
                        c.getCharacter().getMap().spawnItemDrop(c.getCharacter(), c.getCharacter(), item, new Point(dStart, c.getCharacter().getPosition().y), false, false);
                        dStart += 32;
    ...
    public static class Spawn extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (onemob == null) {
                c.getCharacter().dropMessage(5, "Mob does not exist");
                return 0;
            }
            long newhp;
            int newexp;
            ...
            for (int i = 0; i < num; i++) {
                ...
                c.getCharacter().getMap().spawnMonsterOnGroundBelow(mob, c.getCharacter().getPosition());
            }
            return 1;
        }
    }
