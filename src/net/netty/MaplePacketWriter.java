...
public class MaplePacketWriter {
    ...
    public MaplePacketWriter(SendPacketOpcode header) {
    	this(32);
    	this.header = header;
    	writeShort(header.getOpcode());
    }
     ...
     */
    public MaplePacketWriter(final int size) {
    ...
    /**
     * Write a maple-convention ASCII string to the stream with the string length first.
     * @param s
     * @param max
     */
	...
	public SendPacketOpcode getHeader() {
		return header;
	}
	...
	public void print() {
		if (header != null) {
	        ...
	        for (int i = 4; i > Integer.valueOf(opcode.length() / 8); i--) {
	            tab += "\t";
	        }
	        System.out.println("[Send]\t" + opcode + tab + "|     " + toString()/* + "\r\nCaller: " + Thread.currentThread().getStackTrace()[2] */);
		}
	}
