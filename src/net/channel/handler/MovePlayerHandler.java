...
public class MovePlayerHandler extends MaplePacketHandler {
	...
	public void handlePacket(final MaplePacketReader inPacket, final MapleClient c, MapleCharacter chr) {
        ...
        List res;
        try {
            res = MovementParse.parseMovement(inPacket, 1, null, chr);
        } catch (ArrayIndexOutOfBoundsException e) {
            System.out.println(new StringBuilder().append("AIOBE Type1:\n").append(inPacket.toString()).toString());
            return;
        }

        if ((res != null) && (c.getCharacter().getMap() != null)) {
        	/*
            if ((inPacket.available() < 11L) || (inPacket.available() > 26L)) {
            	// if (inPacket.available() != 18L) {
                return;
            }
            */
