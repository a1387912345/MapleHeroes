...
public class UserInterfaceHandler {
    ...
    public static final void CygnusSummon_NPCRequest(final MapleClient c) {
        if (c.getCharacter().getJob() == 2000) {
            NPCScriptManager.getInstance().start(c, 1202000, null);
        } else if (c.getCharacter().getJob() == 1000) {
            NPCScriptManager.getInstance().start(c, 1101008, null);
        }
    }

    public static final void InGame_Poll(final MaplePacketReader inPacket, final MapleClient c) {
        ...
        if (PollEnabled) {
            ...
            final int selection = inPacket.readInt();

            if (selection >= 0 && selection <= Poll_Answers.length) {
                //if (MapleCharacterUtil.SetPoll(c.getAccID(), selection)) {
                //c.sendPacket(CField.getPollReply("Thank you.")); //idk what goes here lol
                //}
            }
        }
    }
