...
public class PartyHandler {
    ...
    public static void MemberSearch(MaplePacketReader inPacket, MapleClient c) {
        ...
        c.sendPacket(CWvsContext.PartyPacket.showMemberSearch(c.getCharacter().getMap().getCharactersThreadsafe()));
    }

    public static final void PartySearch(MaplePacketReader inPacket, MapleClient c) {
        if ((c.getCharacter().isInBlockedMap()) || (FieldLimitType.VipRock.check(c.getCharacter().getMap().getFieldLimit()))) {
            c.getCharacter().dropMessage(5, "You may not do party search here.");
            return;
        }
    ...
    public static final void PartyListing(MaplePacketReader inPacket, MapleClient c) {
        int mode = inPacket.readByte();
        PartySearchType pst;
        switch (mode) {
            case -105:
            case -97:
            case 81:
            case 159:
                pst = PartySearchType.getById(inPacket.readInt());
                ...
                if ((c.getCharacter().getParty() == null) && (World.Party.searchParty(pst).size() < 10)) {
                    ...
                    PartySearch ps = new PartySearch(inPacket.readMapleAsciiString(), pst.exped ? party.getExpeditionId() : party.getId(), pst);
                ...
                if (party != null) {
                    ...
                    if (exped != null) {
                        ...
                        if ((ps != null) && (c.getCharacter().getLevel() <= ps.getType().maxLevel) && (c.getCharacter().getLevel() >= ps.getType().minLevel) && (exped.getAllMembers() < exped.getType().maxMembers)) {
                            ...
                            c.sendPacket(CWvsContext.ExpeditionPacket.expeditionError(0, c.getCharacter().getName()));
                        }
                    }
                }
                break;
    ...
    public static final void Expedition(MaplePacketReader inPacket, MapleClient c) {
        ...
        switch (mode) {
            case 76://64
            case 134:
                ExpeditionType et = ExpeditionType.getById(inPacket.readInt());
                ...
                if (theCh > 0) {
                    ...
                    if ((invited != null) && (invited.getParty() == null) && (party != null) && (party.getExpeditionId() > 0)) {
                        ...
                        if ((me != null) && (me.getAllMembers() < me.getType().maxMembers) && (invited.getLevel() <= me.getType().maxLevel) && (invited.getLevel() >= me.getType().minLevel)) {
                            ...
                            invited.getClient().sendPacket(CWvsContext.ExpeditionPacket.expeditionInvite(c.getCharacter(), me.getType().exped));
                        } else {
            ...
            case 136:
                ...
                if ((cfrom != null) && (cfrom.getParty() != null) && (cfrom.getParty().getExpeditionId() > 0)) {
                    ...
                    if ((exped != null) && (action == 8)) {
                        ...
                        cfrom.getClient().sendPacket(CWvsContext.PartyPacket.partyStatusMessage(23, c.getCharacter().getName()));
                    }
                }
                break;
            case 79://67
            ...
            case 139:
                ...
                exped = World.Party.getExped(part.getExpeditionId());
                if ((exped != null) && (exped.getLeader() == c.getCharacter().getId())) {
                    MaplePartyCharacter newleader = part.getMemberById(inPacket.readInt());
                    if (newleader != null) {
            ...
            case 141:
                ...
                exped = World.Party.getExped(part.getExpeditionId());
                if ((exped != null) && (exped.getLeader() == c.getCharacter().getId())) {
                    int partyIndexTo = inPacket.readInt();
