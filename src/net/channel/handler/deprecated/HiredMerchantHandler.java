...
public class HiredMerchantHandler {
        ...
        if (merch) {
            ...
            if (pack == null) {
                ...
                if (deletePackage(c.getCharacter().getAccountID(), pack.getPackageid(), c.getCharacter().getId())) {
                    ...
                    c.sendPacket(NPCPacket.getNPCTalk(9030000, (byte) 0, "I see that you forgot something here right?\r\nHere is your money sir " + pack.getMesos(), "00 00", (byte) 0));
                    c.getCharacter().setConversation(0);
                } else {
                    c.getCharacter().dropMessage(1, "An unknown error occured.");
                }
               ...
               c.sendPacket(PlayerShopPacket.merchItemStore_ItemData(pack));
                ...
                for (final Item item : pack.getItems()) {
                    if(c.getCharacter().getInventory(GameConstants.getInventoryType(item.getItemId())).isFull()){
                        c.sendPacket(NPCPacket.getNPCTalk(9030000, (byte) 0, "Sir, if you want your items back please clean up your inventory before you come here!", "00 00", (byte) 0));
                        c.getCharacter().setConversation(0);
                        break;
                    }
                    MapleInventoryManipulator.addFromDrop(c, item, true);
                    deletePackage(c.getCharacter().getAccountID(), pack.getPackageid(), c.getCharacter().getId());
                    c.sendPacket(NPCPacket.getNPCTalk(9030000, (byte) 0, "I saved your items sir, next time don't forget them, have a nice day.", "00 00", (byte) 0));
                    c.getCharacter().setConversation(0);
                }
    ...
    public static void displayMerch2(MapleClient c) {
        ...
        if (merch) {
            ...
            if (pack == null) {
                c.sendPacket(NPCPacket.getNPCTalk(9030000, (byte) 0, "I don't think you have any items or money to retrive here. This is where you retrieve the items and mesos that you couldn't get from your Hired Merchant. You'll also need to see me as the character that opened the Personal Store.", "00 00", (byte) 0));
                c.getCharacter().setConversation(0);
            } else if (pack.getItems().size() <= 0) { //error fix for complainers.
                if (!check(c.getCharacter(), pack)) {
                    c.sendPacket(PlayerShopPacket.merchItem_Message((byte) 0x21));
                ...
                for (final Item item : pack.getItems()) {
                    if (c.getCharacter().getInventory(GameConstants.getInventoryType(item.getItemId())).isFull()) {
                        c.sendPacket(NPCPacket.getNPCTalk(9030000, (byte) 0, "Please clean up your inventory.", "00 00", (byte) 0));
                        c.getCharacter().setConversation(0);
                        break;
                    }
                    MapleInventoryManipulator.addFromDrop(c, item, true);
                    deletePackage(c.getCharacter().getAccountID(), pack.getPackageid(), c.getCharacter().getId());
                    //c.getPlayer().fakeRelog();
                    c.sendPacket(NPCPacket.getNPCTalk(9030000, (byte) 0, "Your items have been claimed.", "00 00", (byte) 0));
                    c.getCharacter().setConversation(0);
                }
    ...
    public static final void MerchantItemStore(final MaplePacketReader inPacket, final MapleClient c) {
        if (c.getCharacter() == null) {
            return;
        }
        final byte operation = inPacket.readByte();
