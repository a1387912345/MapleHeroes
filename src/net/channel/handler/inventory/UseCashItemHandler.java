...
public class UseCashItemHandler extends MaplePacketHandler {
        ...
        switch (itemId) {
            ...
            case 5050000: { // AP Reset
                ...
                if (used) {
                    ...
                    c.sendPacket(CWvsContext.updatePlayerStats(statupdate, true, c.getCharacter()));
                }
                break;
            }
            //case 5051001: {
                    ...
                    if (eq.getState() >= 17 && eq.getState() != 20) {
                        ...
                        if (tofind != 0) {
                            ...
                            if (magnifyEquip(c, magnify, item, (byte) item.getPosition())) {
                                ...
                                c.sendPacket(CField.getGameMessage((short) 7, "A Magnifying Glass (Premium) has been used."));
                            } else {
                                c.sendPacket(CField.getGameMessage((short) 7, "A Magnifying Glass was not found. The equipment will stay as Hidden Potential."));
                            }
                        } else {
                            c.sendPacket(CField.getGameMessage((short) 7, "A Magnifying Glass was not found. The equipment will stay as Hidden Potential."));
                        }
                        MapleInventoryManipulator.addById(c, 2430112, (short) 1, "Cube" + " on " + FileoutputUtil.CurrentReadable_Date());
                        c.sendPacket(CField.enchantResult(tofind == 0 ? 1 : 1));//3
                        c.sendPacket(CWvsContext.enableActions());
                        used = true;
                    } else {
                        c.getCharacter().dropMessage(5, "This item's Potential cannot be reset.");
            ...
            case 5521000: { // Karma
                ...
                if (item != null && !ItemFlag.KARMA_ACC.check(item.getFlag())
                    ...
                    if (MapleItemInformationProvider.getInstance().isShareTagEnabled(item.getItemId())) {
                        ...
                        c.sendPacket(InventoryPacket.updateSpecialItemUse(item, type.getType(), item.getPosition(), true, c.getCharacter()));
                        used = true;
                    }
                }
                break;
            ...
            case 5190000: { // Pet Flags
                ...
                if (zz != null && !zz.check(pet.getFlags())) {
                    ...
                    c.sendPacket(PetPacket.updatePet(pet, c.getCharacter().getInventory(MapleInventoryType.CASH).getItem((byte) pet.getInventoryPosition()), true));
                    c.sendPacket(CWvsContext.enableActions());
	...
	private static boolean magnifyEquip(final MapleClient c, Item magnify, Item toReveal, byte eqSlot) {
        ...
        if (eqq.getState() == 1 && (insight || magnify.getItemId() == 2460003 || (magnify.getItemId() == 2460002 && reqLevel <= 12) || (magnify.getItemId() == 2460001 && reqLevel <= 7) || (magnify.getItemId() == 2460000 && reqLevel <= 3))) {
            ...
            if (!insight) {
                c.sendPacket(InventoryPacket.scrolledItem(magnify, eqSlot >= 0 ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED, toReveal, false, true, false));
            } else {
	...
	private static boolean getIncubatedItems(MapleClient c, int itemId) {
        ...
        c.sendPacket(NPCPacket.getNPCTalk(1090000, (byte) 0, "You have obtained the following items:\r\n#i" + id1 + "##z" + id1 + "#\r\n#i" + id2 + "##z" + id2 + "#", "00 00", (byte) 0));
        return true;
    }
	...
	private static final boolean UsePetFood(MapleClient c, int itemId) {
        ...
        if (pet.getCloseness() < 30000) {
            ...
            if (pet.getCloseness() >= GameConstants.getClosenessNeededForLevel(pet.getLevel() + 1)) {
                ...
                c.sendPacket(EffectPacket.showOwnPetLevelUp(c.getCharacter().getPetIndex(pet)));
                c.getCharacter().getMap().broadcastMessage(PetPacket.showPetLevelUp(c.getCharacter(), petindex));
            }
        }
        c.sendPacket(PetPacket.updatePet(pet, c.getCharacter().getInventory(MapleInventoryType.CASH).getItem(pet.getInventoryPosition()), true));
