...
public class ItemMakerHandler {
    ...
    public static final void StartHarvest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        final MapleReactor reactor = c.getCharacter().getMap().getReactorByOid(slea.readInt());
    ...
    public static void ProfessionInfo(final LittleEndianAccessor slea, final MapleClient c) { //so pointless
        try {
            ...
            if (skillid.startsWith("9200") || skillid.startsWith("9201")) {
                ...
                c.getSession().write(CWvsContext.updateSpecialStat(skillid, level1, level2, c.getCharacter().getHonourNextExp()));
                return;
    ...
    public static final void UsePot(final LittleEndianAccessor slea, final MapleClient c) {
        final int itemid = slea.readInt();
        final Item slot = c.getCharacter().getInventory(MapleInventoryType.USE).getItem(slea.readShort());
        ...
        for (int i = 0; i < c.getCharacter().getImps().length; i++) {
            if (c.getCharacter().getImps()[i] == null) {
                ...
                c.getSession().write(CWvsContext.updateImp(c.getCharacter().getImps()[i], ImpFlag.SUMMONED.getValue(), i, false));
    ...
    public static final void ClearPot(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.getSession().write(CWvsContext.updateImp(c.getCharacter().getImps()[index], ImpFlag.REMOVED.getValue(), index, false));
        c.getCharacter().getImps()[index] = null;
    }

    public static final void FeedPot(final LittleEndianAccessor slea, final MapleClient c) {
        final int itemid = slea.readInt();
        final Item slot = c.getCharacter().getInventory(GameConstants.getInventoryType(itemid)).getItem((short) slea.readInt());
        ...
        c.getSession().write(CWvsContext.updateImp(c.getCharacter().getImps()[index], mask, index, false));
    }

    public static final void CurePot(final LittleEndianAccessor slea, final MapleClient c) {
        final int itemid = slea.readInt();
        final Item slot = c.getCharacter().getInventory(MapleInventoryType.ETC).getItem((short) slea.readInt());
        ...
        c.getCharacter().getImps()[index].setState(1);
        c.getSession().write(CWvsContext.updateImp(c.getCharacter().getImps()[index], ImpFlag.STATE.getValue(), index, false));
