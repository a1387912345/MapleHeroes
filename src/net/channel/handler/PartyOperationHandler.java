        ...
        switch (operation) {
                ...
                if (partyInvitee != null) {
                	if ((partyInvitee.getParty() == null) && (partyInvitee.getQuestNoAdd(MapleQuest.getInstance(122901)) == null)) {
                        ...
                        if (party.getMembers().size() < 8) {
                            //c.sendPacket(CWvsContext.PartyPacket.partyStatusMessage(30, partyInvitee.getName()));
                        	c.sendPacket(CWvsContext.PartyPacket.partyRequestInvite(partyInvitee));
                            partyInvitee.getClient().sendPacket(CWvsContext.PartyPacket.partyInvite(c.getCharacter()));
                        } else {
                            c.sendPacket(CWvsContext.PartyPacket.partyStatusMessage(22, null));
                        }
                    } else {
            ...
            case 66://was 7
                ...
                if ((cfrom != null) && (cfrom.getQuestNoAdd(MapleQuest.getInstance(122900)) == null)) {
                    c.sendPacket(CWvsContext.PartyPacket.partyStatusMessage(50, c.getCharacter().getName()));
                    cfrom.getClient().sendPacket(CWvsContext.PartyPacket.partyRequestInvite(c.getCharacter()));
                } else {
