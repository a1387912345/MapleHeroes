...
//      }
    ...
    public static AttackInfo parseMagicDamage(LittleEndianAccessor in, MapleCharacter chr) {
        AttackInfo ret = new AttackInfo();

        in.readByte();
        ret.tbyte = in.readByte();
        ret.targets = ((byte) (ret.tbyte >>> 4 & 0xF));
        ret.hits = ((byte) (ret.tbyte & 0xF));
        ret.skillid = in.readInt();
        if (ret.skillid >= 91000000 && ret.skillid < 100000000) {
            return null;
        }
        
        ret.skillLevel = in.readByte();
        in.readInt(); // SkillLevelData::GetCrc
        if (GameConstants.isMagicChargeSkill(ret.skillid)) {
            ret.charge = in.readInt();
        } else {
            ret.charge = -1;
        }
        ret.flag = in.readByte(); 
        ret.unk = in.readByte();
        ret.display = in.readUShort();
        ...
        ret.speed = in.readByte();
        ret.lastAttackTickCount = in.readInt();
        in.readInt();
        ...
        for (int i = 0; i < ret.targets; i++) {
            int oid = in.readInt();                
            byte unkByte = in.readByte();
            byte unkBool1 = in.readByte();
            byte unkBool2 = in.readByte();
            in.readByte();
            in.readByte();
            in.readInt(); // Monster ID
            in.readByte();
            in.readShort();
            in.readShort();
            in.readShort();
 ...
 //   } 
     ...
     */
    public static AttackInfo parseRangeDamage(LittleEndianAccessor mpr, MapleCharacter chr) {
        AttackInfo ret = new AttackInfo();
