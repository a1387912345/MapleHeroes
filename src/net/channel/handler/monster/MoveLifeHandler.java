...
public class MoveLifeHandler extends MaplePacketHandler {
	...
	public void handlePacket(final MaplePacketReader inPacket, final MapleClient c, MapleCharacter chr) {
		final int oid = inPacket.readInt();
        ...
        inPacket.skip(1); // Azwan boolean
        final short moveid = inPacket.readShort();
        final boolean useSkill = inPacket.readByte() == 1;
        final byte skill = inPacket.readByte();
        final int skill1 = inPacket.readByte() & 0xFF; // unsigned?
        final int skill2 = inPacket.readByte() & 0xFF;
        final int skill3 = inPacket.readByte();
        final int skill4 = inPacket.readByte();
        int realskill = 0;
        int level = 0;
        /*if (!useSkill && skill != 0) {
        ...
        List res;
        res = MovementParse.parseMovement(inPacket, 2, startPos);

        if (monster != null && c != null) {
            c.sendPacket(MobPacket.moveMonsterResponse(oid, moveid, monster.getMp(), monster.isControllerHasAggro(), realskill, level));
        }
        if (res != null) {
