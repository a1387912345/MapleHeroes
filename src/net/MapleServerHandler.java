...
// <editor-fold defaultstate="collapsed" desc="Packet Log Implementation">
        ...
        try {
            ...
            if (logged == null) {
                logged = new LoggedPacket(packet, op, io.getRemoteAddress().toString(),
                        c == null ? -1 : c.getAccountID(), FileoutputUtil.CurrentReadable_Time(),
                        c == null || c.getAccountName() == null ? "[Null]" : c.getAccountName(),
                        c == null || c.getCharacter() == null || c.getCharacter().getName() == null ? "[Null]" : c.getCharacter().getName(),
                        c == null || c.getCharacter() == null || c.getCharacter().getMap() == null ? "[Null]" : String.valueOf(c.getCharacter().getMapId()),
    ...
    public void exceptionCaught(final IoSession session, final Throwable cause) throws Exception {
        ...
        if (c != null && c.getCharacter() != null) {
            FilePrinter.printError(FilePrinter.EXCEPTION_CAUGHT, cause, "Exception caught by: " + c.getCharacter());
        }
    }
    ...
    public void sessionOpened(final IoSession session) throws Exception {
        ...
        final MapleClient client = new MapleClient(
                new MapleCrypto(ServerConfig.USE_FIXED_IV ? ServerConfig.Static_LocalIV : ivSend, (short) (0xFFFF - ServerConstants.CLIENT_VERSION)), // Sent Cypher
                new MapleCrypto(ServerConfig.USE_FIXED_IV ? ServerConfig.Static_RemoteIV : ivRecv, ServerConstants.CLIENT_VERSION), // Recv Cypher
                session);
        client.setChannel(-1);
    ...
    public void sessionClosed(final IoSession session) throws Exception {
        ...
        if (client != null) {
            ...
            if (Log_Packets && !LoginServer.isShutdown() && client.getCharacter() != null) {
    ...
    */
    ...
    public void messageReceived(IoSession session, Object message) {
        ...
        final MaplePacketHandler packetHandler = PacketProcessor.getInstance().getHandler(packetHeader);
        if (packetHandler != null && packetHandler.validateState(client)) {
        	if (ServerConfig.logPackets && !RecvPacketOpcode.isSpam(packetHandler.getRecvOpcode())) {
        		printRecvPacket(packetHandler.getRecvOpcode().name(), packetHeader, (byte[]) message);
                //System.out.println(HexTool.toStringFromAscii((byte[]) message)); 
        	}
            try {
                packetHandler.handlePacket(lea, client, client.getCharacter());
            } catch (final Throwable t) {
            	System.err.println("[Error] An error occured when trying to handle the packet " + HexTool.getOpcodeToString(packetHeader));
            	t.printStackTrace();
                FilePrinter.printError(FilePrinter.PACKET_HANDLER + packetHandler.getClass().getName() + ".txt", t, "Error for " + (client.getCharacter() == null ? "" : "player ; " + client.getCharacter() + " on map ; " + client.getCharacter().getMapId() + " - ") + "account ; " + client.getAccountName() + "\r\n" + lea.toString());
            }
        } else if (ServerConfig.logPackets && !RecvPacketOpcode.isSpam(packetHandler.getRecvOpcode())){
        	String recvName = "UNKNOWN";
        	for (RecvPacketOpcode recv : RecvPacketOpcode.values()) {
        		if (recv.getOpcode() == packetHeader) {
        			recvName = recv.name();
        		}
        	}
        	printRecvPacket(recvName, packetHeader, (byte[]) message);
            System.out.println(HexTool.toStringFromAscii((byte[]) message)); 
        }
    }
    ...
    public static void handlePacket(final RecvPacketOpcode header, final LittleEndianAccessor slea, final MapleClient c) throws Exception {
        if (ServerConfig.logPackets && !isSpamHeader(header)) {
            ...
            System.out.println("[Recv]\t" + header.name() + tab + "|\t" + header.getOpcode() + "\t|\t" + HexTool.getOpcodeToString(header.getOpcode()));
            FileoutputUtil.log("PacketLog.txt", "\r\n\r\n[Recv]\t" + header.name() + tab + "|\t" + header.getOpcode() + "\t|\t" + HexTool.getOpcodeToString(header.getOpcode()) + "\r\n\r\n");
        }
        switch (header) {
        /*    case LOGIN_REDIRECTOR:
        	...
        	case LOGIN_REDIRECTOR:
                  ...
                  //  client_password = slea.readMapleAsciiString();
                    ...
                    c.getSession().write(LoginPacket.getLoginSuccess(c));
...
//                System.out.println("[CLIENT ERROR] Error Code " + errortype + " Type " + type_str + "\r\n\t[Data] Length " + data_length + " [" + SendPacketOpcode.getOpcodeName(opcodeheader) + " | " + opcodeheader + "]\r\n" + HexTool.toString(slea.read((int) slea.available())));
                FileoutputUtil.log("ErrorCodes.txt", "Error Type: " + errortype
                        ...
                        + "Error for player ; " + c.getCharacter().getName()
                        + " - account ; " + c.getAccountName() + "\r\n"
                        + SendPacketOpcode.getOpcodeName(opcodeheader)
                        + " Opcode: " + opcodeheader + "\r\n"
                        + HexTool.toString(slea.read((int) slea.available()))
                        + "\r\n MapID: " + c.getCharacter().getMapId() + "\r\n\r\n");
                break;
            case ENABLE_SPECIAL_CREATION:
                c.getSession().write(LoginPacket.enableSpecialCreation(c.getAccountID(), true));
                break;
            // END OF LOGIN SERVER;
            ...
            case PROFESSION_INFO:
                ...
                break;
            case CRAFT_DONE:
                ItemMakerHandler.CraftComplete(slea, c, c.getCharacter());
                break;
            case CRAFT_MAKE:
                ItemMakerHandler.CraftMake(slea, c, c.getCharacter());
                break;
            case CRAFT_EFFECT:
                ItemMakerHandler.CraftEffect(slea, c, c.getCharacter());
                break;
            case START_HARVEST:
                ItemMakerHandler.StartHarvest(slea, c, c.getCharacter());
                break;
            case STOP_HARVEST:
                ItemMakerHandler.StopHarvest(slea, c, c.getCharacter());
                break;
            case MAKE_EXTRACTOR:
                ItemMakerHandler.MakeExtractor(slea, c, c.getCharacter());
                break;
            case USE_TITLE:
                PlayerHandler.UseTitle(slea.readInt(), c, c.getCharacter());
                break;
            case ANGELIC_CHANGE:
                PlayerHandler.AngelicChange(slea, c, c.getCharacter());
                break;
            case DRESSUP_TIME:
                PlayerHandler.DressUpTime(slea, c, c.getCharacter());
                break;


            case SKILL_EFFECT:
                PlayerHandler.SkillEffect(slea, c.getCharacter());
                break;
            case QUICK_SLOT:
                PlayerHandler.QuickSlot(slea, c.getCharacter());
                break;
            ...
            case LIE_DETECTOR_RESPONSE:
                ...
                break;
            case ARAN_COMBO:
                PlayerHandler.AranCombo(c, c.getCharacter(), 1);
               // CField.updateCombo(+1);
                break;
            case REMOVE_ARAN_COMBO:
                PlayerHandler.RemoveAranCombo(c.getCharacter());
                break;
            case TRANSFORM_PLAYER:
                PlayersHandler.TransformPlayer(slea, c, c.getCharacter());
                break;
            case NOTE_ACTION:
                PlayersHandler.Note(slea, c.getCharacter());
                break;
            case MOVE_BAG:
            ...
            case REWARD_ITEM:
                InventoryHandler.UseRewardItem(slea, c, c.getCharacter());
                break;
            case SOLOMON_EXP:
                InventoryHandler.UseExpItem(slea, c, c.getCharacter());
                break;
            ...
            case ADMIN_COMMAND:
                PlayerHandler.AdminCommand(slea, c, c.getCharacter());
                break;
            case ADMIN_LOG:
                CommandProcessor.logCommandToDB(c.getCharacter(), slea.readMapleAsciiString(), "adminlog");
                break;
            case QUICK_MOVE:
            ...
            case SHIP_OBJECT:
                ...
                break;
            case BUY_CS_ITEM:
                CashShopOperation.BuyCashItem(slea, c, c.getCharacter());
                break;
            case COUPON_CODE:
                slea.skip(2);
...
//                CashShopOperation.doCSPackets(c);
            ...
            case TWIN_DRAGON_EGG:
                ...
                Item itemz = c.getCharacter().getCashInventory().toItem(item);
            ...
            case START_EVOLUTION:
                PlayersHandler.startEvo(slea, c.getCharacter(), c);
                break;
            case ZERO_TAG:
            ...
            case MESSENGER_RANKING:
                PlayerHandler.MessengerRanking(slea, c, c.getCharacter());
                break;
            case OS_INFORMATION:
