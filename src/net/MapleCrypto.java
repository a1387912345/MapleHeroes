...
public class MapleCrypto {
	...
	public static final byte[] MAPLE_AES_KEY = new byte[]{
			(byte) 0xBB, (byte) 0x00, (byte) 0x00, (byte) 0x00, 
			(byte) 0x72, (byte) 0x00, (byte) 0x00, (byte) 0x00, 
			(byte) 0x46, (byte) 0x00, (byte) 0x00, (byte) 0x00, 
			(byte) 0x9B, (byte) 0x00, (byte) 0x00, (byte) 0x00, 
			(byte) 0x72, (byte) 0x00, (byte) 0x00, (byte) 0x00, 
			(byte) 0xC9, (byte) 0x00, (byte) 0x00, (byte) 0x00, 
			(byte) 0x71, (byte) 0x00, (byte) 0x00, (byte) 0x00, 
			(byte) 0x80, (byte) 0x00, (byte) 0x00, (byte) 0x00
	};
	private static final SecretKeySpec KEY_SPEC = new SecretKeySpec(MAPLE_AES_KEY, "AES");
	private static final ThreadLocal<Cipher> CIPHER = new ThreadLocal<Cipher>() {
		...
		protected Cipher initialValue() {
			try {
				...
				System.err.println("[Error] Error initializing the encryption cipher. Please make sure you are using the Unlimited Strength cryptography jar files.");
			}
			return null;
		}
	};
            ...
            while (remaining > 0) {
                ...
                for (int x = start; x < (start + llength); x++) {
                    if ((x - start) % myIv.length == 0) {
                        byte[] newIv = CIPHER.get().doFinal(myIv);
     ...
     */
    public static byte[] shuffle(byte inputByte, byte[] in) {
        byte elina = in[1];
        final byte anna = inputByte;
        byte moritz = bShuffle[elina & 0xFF];
        moritz -= inputByte;
        in[0] += moritz;
        moritz = in[2];
        moritz ^= bShuffle[anna & 0xFF];
        elina -= moritz & 0xFF;
        in[1] = elina;
        elina = in[3];
        moritz = elina;
        elina -= in[0] & 0xFF;
        moritz = bShuffle[moritz & 0xFF];
        moritz += inputByte;
        moritz ^= in[2];
        in[2] = moritz;
        elina += bShuffle[anna & 0xFF] & 0xFF;
        in[3] = elina;
