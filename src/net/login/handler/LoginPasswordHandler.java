	...
	public void handlePacket(final MaplePacketReader lea, final MapleClient c, MapleCharacter chr) {
        ...
        if (AutoRegister.autoRegister && !AutoRegister.getAccountExists(login) && (!c.hasBannedIP() || !c.hasBannedMac())) {
            if (pwd.equalsIgnoreCase("disconnect") || pwd.equalsIgnoreCase("fixme")) {
                c.sendPacket(CWvsContext.broadcastMsg(1, "This password is invalid."));
                c.sendPacket(LoginPacket.getLoginFailed(1)); //Shows no message, used for unstuck the login button
                return;
            }
            AutoRegister.createAccount(login, pwd, c.getSocketChannel().remoteAddress().toString());
            if (AutoRegister.success) {
                c.sendPacket(CWvsContext.broadcastMsg(1, "Account has been successfully registered!\r\nPlease login again to enter your new account."));
                c.sendPacket(LoginPacket.getLoginFailed(1)); //Shows no message, used for unstuck the login button
                return;
            }
            ...
            c.sendPacket(CWvsContext.broadcastMsg(1, "Your characters have been disconnected successfully."));
            c.sendPacket(LoginPacket.getLoginFailed(1)); //Shows no message, used for unstuck the login button
            return;
        } else {
        ...
        if (!c.isGm() && !c.isLocalhost() && ServerConstants.Use_Localhost) {
            c.sendPacket(CWvsContext.broadcastMsg(1, "We are sorry, but the server is under a maintenance, please check the forums for more information."));
            c.sendPacket(LoginPacket.getLoginFailed(1)); //Shows no message, used for unstuck the login button
        }
        ...
        if (loginok == 0 && (ipBan || macBan) && !c.isGm()) {
            ...
            if (macBan) {
                ...
                MapleCharacter.ban(c.getSocketChannel().remoteAddress().toString().split(":")[0], "Enforcing account ban, account " + login, false, 4, false);
            }
        }
        if (loginok != 0) {
            if (!loginFailCount(c)) {
