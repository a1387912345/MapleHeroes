...
public class CharLoginHandler {
    ...
    public static void redirectorLogin(final MaplePacketReader inPacket, final MapleClient c) {
        String username = inPacket.readMapleAsciiString();
        String password = inPacket.readMapleAsciiString(); //this LOL
    ...
    public static void CreateChar(final MaplePacketReader inPacket, final MapleClient c) {
        if (!c.isLoggedIn()) {
            c.close();
            return;
        }
        final String name = inPacket.readMapleAsciiString();
        inPacket.readInt(); // key settings
        inPacket.readInt(); // -1 ?
        final JobType jobType = JobType.getByType(inPacket.readInt()); // BIGBANG: 0 = Resistance, 1 = Adventurer, 2 = Cygnus, 3 = Aran, 4 = Evan, 5 = mercedes
        final short subcategory = inPacket.readShort(); //whether dual blade = 1 or adventurer = 0
        final byte gender = inPacket.readByte(); //??idk corresponds with the thing in addCharStats
        byte skinColor = inPacket.readByte(); // 01
        int hairColor = 0;
        final byte unk = inPacket.readByte(); // 08
        ...
        final int face = inPacket.readInt();
        final int hair = inPacket.readInt();
        if (!mercedes && !mihile && !demon && !jettPhantom) { //mercedes/demon dont need hair color since its already in the hair
            if (!adv) {
                hairColor = inPacket.readInt();
            }
            skinColor = (byte) inPacket.readInt();
        }
        if(mihile){ //For some reason an int was missing.
            final int unk3 = inPacket.readInt();
        }
        final int faceMark = (aran ? 0 : evan ? 0 : resist ? 0 : cygnus ? 0 : adv ? 0 : inPacket.readInt());
        final int top = inPacket.readInt();
        final int bottom = adv ? 0 : inPacket.readInt();
        final int shoes = inPacket.readInt();
        final int weapon = jobType == LoginInformationProvider.JobType.Phantom ? 1362046 : mercedes ? 1522038 : evan ? 1372005 : inPacket.readInt();
        int shield = jobType == LoginInformationProvider.JobType.Phantom ? 1352100 : jobType == LoginInformationProvider.JobType.Mihile ? 1098000 : mercedes ? 1352000 : demon ? inPacket.readInt() : 0;
    ...
    public static void Character_login_noPIC(final MaplePacketReader inPacket, final MapleClient c, final boolean view, final boolean haspic) {
        if (haspic) {
            final String password = inPacket.readMapleAsciiString();
            final int charId = inPacket.readInt();
            if (view) {
                c.setChannel(1);
                c.setWorld(inPacket.readInt());
            }
            c.updateMacs(inPacket.readMapleAsciiString());
            final String s = c.getSessionIPAddress();
            LoginServer.getInstance().putLoginAuth(charId, s.substring(s.indexOf('/') + 1, s.length()), c.getTempIP(), c.getChannel());
            c.updateLoginState(MapleClient.LOGIN_SERVER_TRANSITION, s);
            c.sendPacket(CField.getServerIP(Integer.parseInt(ChannelServer.getInstance(c.getChannel()).getIP().split(":")[1]), c.getWorld(), charId));
        } else {
            inPacket.readByte(); // 1?
            inPacket.readByte(); // 1?
            final int charId = inPacket.readInt();
    ...
    public static void Character_WithoutSecondPassword(final MaplePacketReader inPacket, final MapleClient c, final boolean haspic, final boolean view) {
        if (constants.ServerConfig.DISABLE_PIC) {
            Character_login_noPIC(inPacket, c, view, haspic);
        }
        ...
        final String currentpw = c.getPIC();
        
        if (!c.isLoggedIn() || loginFailCount(c) || (currentpw != null && (!currentpw.equals("") || haspic)) || !c.loginAuth(charId) || ChannelServer.getInstance(c.getChannel()) == null || !WorldOption.isExists(c.getWorld())) {
            c.close();
            return;
        }
        ...
        if (inPacket.available() != 0) {
            final String setpassword = inPacket.readMapleAsciiString();

            if (setpassword.length() >= 6 && setpassword.length() <= 16) {
                c.setPIC(setpassword);
                c.updatePIC();
            } else {
    ...
    public static void Character_WithSecondPassword(final MaplePacketReader inPacket, final MapleClient c, final boolean view) {
        if (constants.ServerConfig.DISABLE_PIC) {
            Character_login_noPIC(inPacket, c, view, true);
        }
        final String password = inPacket.readMapleAsciiString();
        final int charId = inPacket.readInt();
        if (view) {
            c.setChannel(1);
            c.setWorld(inPacket.readByte());
        }
        if (!c.isLoggedIn() || loginFailCount(c) || c.getPIC() == null || !c.loginAuth(charId) || ChannelServer.getInstance(c.getChannel()) == null || !WorldOption.isExists(c.getWorld())) {
            c.close();
            return;
        }
        c.updateMacs(inPacket.readMapleAsciiString());
        if (c.checkPIC(password) && password.length() >= 6 && password.length() <= 16 || c.isGm() || c.isLocalhost()) {
    ...
    public static void partTimeJob(final MaplePacketReader inPacket, final MapleClient c) {
        System.out.println("[Part Time Job] data: " + inPacket);
        byte mode = inPacket.readByte(); //1 = start 2 = end
        int cid = inPacket.readInt(); //character id
        byte job = inPacket.readByte(); //part time job
        if (mode == 0) {
