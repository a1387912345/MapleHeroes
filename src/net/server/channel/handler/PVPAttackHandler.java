...
public class PVPAttackHandler extends MaplePacketHandler {
	...
	public void handlePacket(MaplePacketReader lea, MapleClient client, MapleCharacter chr) {
        ...
        if (skillid > 0) {
            ...
            if (effect.getCooldown(chr) > 0 && !chr.isGM()) {
                ...
                if ((skillid != 35111004 && skillid != 35121013) || chr.getBuffSource(MapleBuffStat.MECH_CHANGE) != skillid) { // Battleship
                    client.sendPacket(CField.skillCooldown(skillid, effect.getCooldown(chr)));
        ...
        if (didAttack) {
            ...
            PlayerHandler.AranCombo(client, chr, ourAttacks.size() * attackCount);
