...
import net.server.channel.handler.deprecated.AttackType;
import net.server.channel.handler.deprecated.DamageParse;
...
public class RangedAttackHandler extends MaplePacketHandler {
...
//                    c.sendPacket(AngelicPacket.showRechargeEffect());
                            ...
                            c.sendPacket(AngelicPacket.lockSkill(attack.skillid));
                        }
                    }
                } else {
                    if (c.getCharacter().isGM()) {
...
//                    c.sendPacket(AngelicPacket.showRechargeEffect());
	            ...
	            for (AttackPair at : attack.allDamage) {
	                ...
	                if (Randomizer.nextInt(100) < percent) {
	                    if (mob != null) {
	                        ...
	                        c.sendPacket(JobPacket.WindArcherPacket.TrifleWind(c.getCharacter().getID(), skillid, count, mob.getObjectId(), type));
	                    }
	                }
	            }
	            }      
            ...
            if (c.getCharacter().getSkillLevel(SkillFactory.getSkill(4100011)) > 0) {
                ...
                if (monsters.size() <= 0) {
                	c.getCharacter().getClient().sendPacket(CWvsContext.enableActions());
                	return;
                	}
                for (AttackPair at : attack.allDamage) {
            ...
            if ((effect.getCooldown(chr) > 0) && (!chr.isGM()) && (((attack.skillid != 35111004) && (attack.skillid != 35121013)) || (chr.getBuffSource(MapleBuffStat.MECH_CHANGE) != attack.skillid))) {
                ...
                c.sendPacket(CField.skillCooldown(attack.skillid, effect.getCooldown(chr)));
                ...
                if ((chr.getJob() == 412) && (bulletConsume > 0) && (ipp.getQuantity() < MapleItemInformationProvider.getInstance().getSlotMax(projectile))) {
                    ...
                    if (chr.getTotalSkillLevel(expert) > 0) {
                        ...
                        if (eff.makeChanceResult()) {
                            ...
                            c.sendPacket(CWvsContext.InventoryPacket.updateInventorySlot(MapleInventoryType.USE, ipp, false));
                            bulletConsume = 0;
        ...
        if (!chr.isHidden()) {
            if (attack.skillid == 3211006) {
                chr.getMap().broadcastMessage(chr, CField.strafeAttack(chr.getID(), attack.tbyte, attack.skillid, attack.skillLevel, attack.display, attack.speed, visProjectile, attack.allDamage, attack.position, chr.getLevel(), chr.getStat().getPassiveMastery(), attack.unk, chr.getTotalSkillLevel(3220010)), chr.getTruePosition());
            } else {
                ...
                chr.getMap().broadcastMessage(chr, CField.rangedAttack(chr.getID(), attack, visProjectile, chr.getLevel(), chr.getStat().getPassiveMastery()), chr.getTruePosition());

            }
        } else if (attack.skillid == 3211006) {
            chr.getMap().broadcastGMMessage(chr, CField.strafeAttack(chr.getID(), attack.tbyte, attack.skillid, attack.skillLevel, attack.display, attack.speed, visProjectile, attack.allDamage, attack.position, chr.getLevel(), chr.getStat().getPassiveMastery(), attack.unk, chr.getTotalSkillLevel(3220010)), false);
        } else {
            chr.getMap().broadcastGMMessage(chr, CField.rangedAttack(chr.getID(), attack.tbyte, attack.skillid, attack.skillLevel, attack.display, attack.speed, visProjectile, attack.allDamage, attack.position, chr.getLevel(), chr.getStat().getPassiveMastery(), attack.unk), false);
        }
