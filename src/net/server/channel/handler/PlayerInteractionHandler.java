...
public class PlayerInteractionHandler extends MaplePacketHandler {
	...
	@Override
	public void handlePacket(final MaplePacketReader mpr, final MapleClient client, final MapleCharacter chr) {
		final Interaction action = Interaction.getByAction(mpr.readByte());
        if (chr == null || action == null) {
            return;
        }
        client.getCharacter().setScrolledPosition((short) 0);
        //System.err.println(action);
        switch (action) { // Mode
            case CREATE: {
                if (chr.getPlayerShop() != null || client.getChannelServer().isShutdown() || chr.hasBlockedInventory()) {
                    System.err.println("amq");
                    client.sendPacket(CWvsContext.enableActions());
                    return;
                }
                final byte createType = mpr.readByte();
                if (createType == 4) { // trade//was 3
                    ...
                    final String desc = mpr.readMapleAsciiString();
                    String pass = "";
                    if (mpr.readByte() > 0) {
                        pass = mpr.readMapleAsciiString();
                    }
                    if (createType == 1 || createType == 2) {
                        final int piece = mpr.readByte();
                        final int itemId = createType == 1 ? (4080000 + piece) : 4080100;
                        if (!chr.haveItem(itemId) || (client.getCharacter().getMapId() >= 910000001 && client.getCharacter().getMapId() <= 910000022)) {
                            return;
                        }
                        ...
                        Item shop = client.getCharacter().getInventory(MapleInventoryType.CASH).getItem((byte) mpr.readShort());
                        if (shop == null || shop.getQuantity() <= 0 || shop.getItemId() != mpr.readInt() || client.getCharacter().getMapId() < 910000001 || client.getCharacter().getMapId() > 910000022) {
                            return;
                        }
                        if (createType == 3) {
                            ...
                            client.sendPacket(PlayerShopPacket.getHiredMerch(chr, merch, true));
                        }
                    }
                }
                break;
            ...
            case INVITE_TRADE: {
                ...
                MapleCharacter chrr = chr.getMap().getCharacterById(mpr.readInt());
                if (chrr == null || client.getChannelServer().isShutdown() || chrr.hasBlockedInventory()) {
                    client.sendPacket(CWvsContext.enableActions());
                    return;
                }
                        ...
                        if (ob instanceof HiredMerchant) {
                            ...
                            if (!merchant.isOpen() || !merchant.isAvailable()) {
                                ...
                                if (ips.getFreeSlot() == -1) {
                                    ...
                                    client.sendPacket(PlayerShopPacket.getHiredMerch(chr, merchant, false));
                                }
                            }
                            //}
                        } else {
                            if (ips instanceof MaplePlayerShop && ((MaplePlayerShop) ips).isBanned(chr.getName())) {
                                ...
                                if (ips.getFreeSlot() < 0 || ips.getVisitorSlot(chr) > -1 || !ips.isOpen() || !ips.isAvailable()) {
                                    client.sendPacket(PlayerShopPacket.getMiniGameFull());
                                } else {
                                    if (mpr.available() > 0 && mpr.readByte() > 0) { //a password has been entered
                                        String pass = mpr.readMapleAsciiString();
                                        if (!pass.equals(ips.getPassword())) {
                                            client.getCharacter().dropMessage(1, "The password you entered is incorrect.");
                                            return;
                                        }
                                    ...
                                    if (ips instanceof MapleMiniGame) {
                                        ((MapleMiniGame) ips).send(client);
                                    } else {
                                        client.sendPacket(PlayerShopPacket.getPlayerStore(chr, false));
                                    }
                                }
                            }
                        }
            ...
            case HIRED_MERCHANT_MAINTENANCE: {
                if (client.getChannelServer().isShutdown() || chr.getMap() == null || chr.getTrade() != null) {
                    client.sendPacket(CWvsContext.enableActions());
                    return;
                }
                mpr.skip(1); // 9?
                byte type = mpr.readByte(); // 6?
                if (type != 6) {
                    client.sendPacket(CWvsContext.enableActions());
                    return;
                }
                final String password = mpr.readMapleAsciiString();
                if (!client.checkPIC(password) || password.length() < 6 || password.length() > 16) {
                ...
                if (shop != null && shop.isOwner(chr) && shop.getShopType() < 3 && !shop.isAvailable()) {
                    if (chr.getMap().allowPersonalShop()) {
                        ...
                        if (shop.getShopType() == 1 && HiredMerchantHandler.UseHiredMerchant(chr.getClient(), false)) {
                            ...
                            merchant.setStoreid(client.getChannelServer().addMerchant(merchant));
...
//            case SET_ITEMS: {
                ...
                final MapleInventoryType ivType = MapleInventoryType.getByType(mpr.readByte());
                final Item item = chr.getInventory(ivType).getItem((byte) mpr.readShort());
                final short quantity = mpr.readShort();
                final byte targetSlot = mpr.readByte();
                ...
                if (chr.getTrade() != null && item != null) {
                    if ((quantity <= item.getQuantity() && quantity >= 0) || GameConstants.isThrowingStar(item.getItemId()) || GameConstants.isBullet(item.getItemId())) {
                        chr.getTrade().setItems(client, item, targetSlot, quantity);
                    }
                }
                break;
            }
...
//            case SET_MESO_2: {
            ...
            case UPDATE_MESO: {
                ...
                if (trade != null) {
                    trade.setMeso((int) mpr.readLong());
                }
                break;
            }
...
//            case ADD_ITEM: {
            ...
            case ADD_ITEM1: {
                ...
                final short bundles = mpr.readShort(); // How many in a bundle
                final short perBundle = mpr.readShort(); // Price per bundle
                final int price = mpr.readInt();
...
//                    }
                    if (ivItem.getQuantity() >= bundles_perbundle) {
                        ...
                        if (bundles_perbundle >= 50 && ivItem.getItemId() == 2340000) {
                            client.setMonitored(true); //hack check
                        }
                        if (GameConstants.getLowestPrice(ivItem.getItemId()) > price) {
                            client.getCharacter().dropMessage(1, "The lowest you can sell this for is " + GameConstants.getLowestPrice(ivItem.getItemId()));
                            client.sendPacket(CWvsContext.enableActions());
                            return;
                        }
                        if (GameConstants.isThrowingStar(ivItem.getItemId()) || GameConstants.isBullet(ivItem.getItemId())) {
                            // Ignore the bundles
                            MapleInventoryManipulator.removeFromSlot(client, type, slot, ivItem.getQuantity(), true);

                            final Item sellItem = ivItem.copy();
                            shop.addItem(new MaplePlayerShopItem(sellItem, (short) 1, price));
                        } else {
                            MapleInventoryManipulator.removeFromSlot(client, type, slot, bundles_perbundle, true);

                            final Item sellItem = ivItem.copy();
                            sellItem.setQuantity(perBundle);
                            shop.addItem(new MaplePlayerShopItem(sellItem, bundles, price));
                        }
                        client.sendPacket(PlayerShopPacket.shopItemUpdate(shop));
                    }
                }
                break;
            }
...
//            case BUY_ITEM_HIREDMERCHANT: { // Buy and Merchant buy
            ...
            case REMOVE_ITEM: {
                ...
                if (item != null) {
                    if (item.bundles > 0) {
                        ...
                        if (MapleInventoryManipulator.checkSpace(client, item_get.getItemId(), item_get.getQuantity(), item_get.getOwner())) {
                            MapleInventoryManipulator.addFromDrop(client, item_get, false);
                            item.bundles = 0;
            ...
            case MAINTANCE_ORGANISE: {
                ...
                if (imps != null && imps.isOwner(chr) && !(imps instanceof MapleMiniGame)) {
                    ...
                    client.sendPacket(PlayerShopPacket.shopItemUpdate(imps));
                }
                break;
            }
            case CLOSE_MERCHANT: {
                ...
                if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr) && merchant.isAvailable()) {
                    ...
                    client.sendPacket(CWvsContext.broadcastMsg(1, "Please visit Fredrick for your items."));
                    client.sendPacket(CWvsContext.enableActions());
                }
                break;
            }
            ...
            case VIEW_MERCHANT_VISITOR: {
                ...
                if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr)) {
                    ((HiredMerchant) merchant).sendVisitor(client);
                }
                break;
            }
            case VIEW_MERCHANT_BLACKLIST: {
                final IMaplePlayerShop merchant = chr.getPlayerShop();
                if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr)) {
                    ((HiredMerchant) merchant).sendBlackList(client);
                }
                break;
            }
            case MERCHANT_BLACKLIST_ADD: {
                final IMaplePlayerShop merchant = chr.getPlayerShop();
                if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr)) {
                    ((HiredMerchant) merchant).addBlackList(mpr.readMapleAsciiString());
                }
                break;
            }
            case MERCHANT_BLACKLIST_REMOVE: {
                final IMaplePlayerShop merchant = chr.getPlayerShop();
                if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr)) {
                    ((HiredMerchant) merchant).removeBlackList(mpr.readMapleAsciiString());
                }
                break;
            }
            case GIVE_UP: {
...
//                        return;
//                    }
//                    game.setPiece(inPacket.readInt(), inPacket.readInt(), inPacket.readByte(), chr);
//                }
//                break;
//            }
//            case SELECT_CARD: {
...
//                        return;
//                    }
//                    if (inPacket.readByte() != game.getTurn()) {
...
//                        return;
//                    }
//                    final int slot = inPacket.readByte();
...
//            }
            default: {
                ...
                //System.out.println("Unhandled interaction action by " + chr.getName() + " : " + action + ", " + inPacket.toString());
