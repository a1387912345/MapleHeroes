...
public class PlayersHandler {
    ...
    public static void Note(final MaplePacketReader inPacket, final MapleCharacter chr) {
        final byte type = inPacket.readByte();

        switch (type) {
            case 0:
                String name = inPacket.readMapleAsciiString();
                String msg = inPacket.readMapleAsciiString();
                boolean fame = inPacket.readByte() > 0;
                inPacket.readInt(); //0?
                Item itemz = chr.getCashInventory().findByCashId((int) inPacket.readLong());
                ...
                break;
            case 1:
                short num = inPacket.readShort();
                if (num < 0) { // note overflow, shouldn't happen much unless > 32767 
                    num = 32767;
                }
                inPacket.skip(1); // first byte = wedding boolean? 
                for (int i = 0; i < num; i++) {
                    final int id = inPacket.readInt();
                    chr.deleteNote(id, inPacket.readByte() > 0 ? 1 : 0);
                }
                break;
            default:
    ...
    public static void DressUpRequest(final MapleCharacter chr, MaplePacketReader inPacket) {
        int code = inPacket.readInt();
        switch (code) {
            case 5010093:
    ...
    public static void TransformPlayer(final MaplePacketReader inPacket, final MapleClient c, final MapleCharacter chr) {
        // D9 A4 FD 00
        // 11 00
        // A0 C0 21 00
        ...
        final String target = inPacket.readMapleAsciiString();
        ...
        public static void startEvo(MaplePacketReader inPacket, MapleCharacter player, MapleClient c) {
   ...
   /*     final List<Integer> maps = new ArrayList<>();
      ...
      //  player.getClient().getChannelServer().getEventSM().getEventManager("EvolutionLab"); //Coming Soon
            ...
            public static void HOLLY(MapleClient c, MaplePacketReader inPacket) {
       final MapleMapObject obj = c.getCharacter().getMap().getMapObject(inPacket.readInt(), MapleMapObjectType.SUMMON);
    ...
    public static void hitCoconut(MaplePacketReader inPacket, MapleClient c) {
        /*CB 00 A6 00 06 01
         * A6 00 = coconut id
         * 06 01 = ?
         */
        int id = inPacket.readShort();
        String co = "coconut";
    ...
    public static void FollowRequest(final MaplePacketReader inPacket, final MapleClient c) {
        MapleCharacter tt = c.getCharacter().getMap().getCharacterById(inPacket.readInt());
        ...
        if (inPacket.readByte() > 0) { //cancelling follow
            tt = c.getCharacter().getMap().getCharacterById(c.getCharacter().getFollowId());
        ...
        if (tt != null && tt.getPosition().distanceSq(c.getCharacter().getPosition()) < 10000 && tt.getFollowId() == 0 && c.getCharacter().getFollowId() == 0 && tt.getID() != c.getCharacter().getID()) { //estimate, should less
            ...
            tt.getClient().sendPacket(CWvsContext.followRequest(c.getCharacter().getID()));
        } else {
            c.sendPacket(CWvsContext.broadcastMsg(1, "You are too far away."));
        }
    }

    public static void FollowReply(final MaplePacketReader inPacket, final MapleClient c) {
        if (c.getCharacter().getFollowId() > 0 && c.getCharacter().getFollowId() == inPacket.readInt()) {
            ...
            if (tt != null && tt.getPosition().distanceSq(c.getCharacter().getPosition()) < 10000 && tt.getFollowId() == 0 && tt.getID() != c.getCharacter().getID()) { //estimate, should less
                boolean accepted = inPacket.readByte() > 0;
                if (accepted) {
                    tt.setFollowId(c.getCharacter().getID());
                    tt.setFollowOn(true);
                    tt.setFollowInitiator(false);
     ...
     //   public static void HOLLY(MapleClient c,  LittleEndianAccessor inPacket) {
   //     int skillid = inPacket.readInt();
   //     if (skillid == 3121013) {
   ...
   //     }
    ...
    public static void DoRing(final MapleClient c, final String name, final int itemid) {
        ...
        chr.getClient().sendPacket(CWvsContext.sendEngagementRequest(c.getCharacter().getName(), c.getCharacter().getID()));
    }

    public static void RingAction(final MaplePacketReader inPacket, final MapleClient c) {
        final byte mode = inPacket.readByte();
        if (mode == 0) {
            DoRing(c, inPacket.readMapleAsciiString(), inPacket.readInt());
            //1112300 + (itemid - 2240004)
        } else if (mode == 1) {
            c.getCharacter().setMarriageItemId(0);
        } else if (mode == 2) { //accept/deny proposal
            final boolean accepted = inPacket.readByte() > 0;
            ...
            if (accepted) {
                ...
                try {
                    ...
                    chr.getClient().sendPacket(CWvsContext.sendEngagement((byte) 0x10, newItemId, chr, c.getCharacter()));
    ...
    public static void Solomon(final MaplePacketReader inPacket, final MapleClient c) {
        c.sendPacket(CWvsContext.enableActions());
        c.getCharacter().updateTick(inPacket.readInt());
        Item item = c.getCharacter().getInventory(MapleInventoryType.USE).getItem(inPacket.readShort());
        if (item == null || item.getItemId() != inPacket.readInt() || item.getQuantity() <= 0 || c.getCharacter().getGachExp() > 0 || c.getCharacter().getLevel() > 50 || MapleItemInformationProvider.getInstance().getItemEffect(item.getItemId()).getEXP() <= 0) {
            return;
        }
    ...
    public static void GachExp(final MaplePacketReader inPacket, final MapleClient c) {
        c.sendPacket(CWvsContext.enableActions());
        c.getCharacter().updateTick(inPacket.readInt());
    ...
    public static void Report(final MaplePacketReader inPacket, final MapleClient c) {
        //0 = success 1 = unable to locate 2 = once a day 3 = you've been reported 4+ = unknown reason
        MapleCharacter other;
        ReportType type;
        type = ReportType.getById(inPacket.readByte());
        other = c.getCharacter().getMap().getCharacterByName(inPacket.readMapleAsciiString());
    ...
    public static void exitSilentCrusadeUI(final MaplePacketReader inPacket, final MapleClient c) {
        c.getCharacter().updateInfoQuest(1652, "alert=-1"); //Hide Silent Crusade icon
    }

    public static void claimSilentCrusadeReward(final MaplePacketReader inPacket, final MapleClient c) {
        short chapter = inPacket.readShort();
    ...
    public static void buySilentCrusade(final MaplePacketReader inPacket, final MapleClient c) {
        //ui window is 0x49
        //inPacket: [00 00] [4F 46 11 00] [01 00]
        short slot = inPacket.readShort(); //slot of item in the silent crusade window
    ...
    public static void UpdatePlayerInformation(final MaplePacketReader inPacket, final MapleClient c) {
        byte mode = inPacket.readByte(); //01 open ui 03 save info
        if (mode == 1) {
        ...
        int country = inPacket.readInt();
        int birthday = inPacket.readInt();
        int favoriteAction = inPacket.readInt(); //kind of mask
        int favoriteLocation = inPacket.readInt(); //kind of mask
    ...
    public static void FindFriends(final MaplePacketReader inPacket, final MapleClient c) {
    ...
    public static void reviveAzwan(MaplePacketReader inPacket, MapleClient c) {
    ...
    public static void magicWheel(MaplePacketReader inPacket, MapleClient c) {
        final byte mode = inPacket.readByte(); // 0 = open 2 = start 4 = receive reward
        if (mode == 2) {
            inPacket.readInt(); //4
            final short toUseSlot = inPacket.readShort();
            inPacket.readShort();
            final int tokenId = inPacket.readInt();
            ...
            final String data = inPacket.readMapleAsciiString();
            int item;
            //try {
    ...
    public static void onReward(MaplePacketReader inPacket, MapleClient c) throws SQLException {
        System.err.println("onReward");
        int id = inPacket.readInt();
        int type = inPacket.readInt();
        int itemId = inPacket.readInt();
        inPacket.readInt(); //might be item quantity
        inPacket.readInt(); //no idea
        inPacket.readLong(); //no idea
        inPacket.readInt(); //no idea
        int mp = inPacket.readInt();
        int meso = inPacket.readInt();
        int exp = inPacket.readInt();
        inPacket.readInt(); //no idea
        inPacket.readInt(); //no idea
        inPacket.readMapleAsciiString(); //no idea
        inPacket.readMapleAsciiString(); //no idea
        inPacket.readMapleAsciiString(); //no idea
        byte mode = inPacket.readByte();
        if (mode == 2) { //Decline
            ...
            if (reward == null) {
                c.sendPacket(Reward.receiveReward(id, (byte) 0x15, 0));
                c.sendPacket(CWvsContext.enableActions());
                return;
            }
            ...
            c.sendPacket(Reward.receiveReward(id, msg, quantity));
        }
        if (mode < 0 || mode > 2) {
    ...
    public static void blackFriday(MaplePacketReader inPacket, MapleClient c) {
    ...
    public static void updateRedLeafHigh(MaplePacketReader inPacket, MapleClient c) { //not finished yet
        //TODO: load and set red leaf high in sql
        inPacket.readInt(); //questid or something
        inPacket.readInt(); //joe joe quest
        int joejoe = inPacket.readInt();
        inPacket.readInt(); //hermoninny quest
        int hermoninny = inPacket.readInt();
        inPacket.readInt(); //little dragon quest
        int littledragon = inPacket.readInt();
        inPacket.readInt(); //ika quest
        int ika = inPacket.readInt();
    ...
    public static void StealSkill(MaplePacketReader inPacket, MapleClient c) {
        if (c.getCharacter() == null || c.getCharacter().getMap() == null || !GameConstants.isPhantom(c.getCharacter().getJob())) {
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        final int skill = inPacket.readInt();
        final int cid = inPacket.readInt();
    ...
    public static void ChooseSkill(MaplePacketReader inPacket, MapleClient c) {
        if (c.getCharacter() == null || c.getCharacter().getMap() == null || !GameConstants.isPhantom(c.getCharacter().getJob())) {
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        final int base = inPacket.readInt();
        final int skill = inPacket.readInt();
        if (skill <= 0) {
    ...
    public static void viewSkills(final MaplePacketReader inPacket, final MapleClient c) {
        int victim = inPacket.readInt();
        ...
        if (!c.getChannelServer().getPlayerStorage().getCharacterById(victim).getSkills().isEmpty() && GameConstants.isAdventurer(jobid)) {
            c.sendPacket(CField.viewSkills(c.getChannelServer().getPlayerStorage().getCharacterById(victim)));
        } else {
    ...
    public static void CassandrasCollection(MaplePacketReader inPacket, MapleClient c) {
        c.sendPacket(CField.getCassandrasCollection());  
    }
    
    public static void calcHyperSkillPointCount(MapleClient c) {
        for (int i = 0; i < 3; i++) {
            c.sendPacket(CWvsContext.updateHyperSp(i, c.getCharacter().getRemainingHSps()[i]));
        }
    }
}
