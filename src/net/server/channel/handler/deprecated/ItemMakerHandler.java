...
public class ItemMakerHandler {
    ...
    public static final void ItemMaker(final MaplePacketReader inPacket, final MapleClient c) {
        //System.out.println(inPacket.toString()); //change?
        final int makerType = inPacket.readInt();

        switch (makerType) {
            case 1: { // Gem
                final int toCreate = inPacket.readInt();
                ...
                if (GameConstants.isGem(toCreate)) {
                    ...
                    if (c.getCharacter().getMeso() < gem.getCost()) {
                        return; // H4x
                    }
                    final int randGemGiven = getRandomGem(gem.getRandomReward());

                    if (c.getCharacter().getInventory(GameConstants.getInventoryType(randGemGiven)).isFull()) {
                    ...
                    c.getCharacter().gainMeso(-gem.getCost(), false);
                    ...
                    c.getCharacter().getMap().broadcastMessage(c.getCharacter(), EffectPacket.ItemMaker_Success_3rdParty(c.getCharacter().getID()), false);
                    ...
                    if (c.getCharacter().getInventory(GameConstants.getInventoryType(toCreate)).isFull()) {
                        return; // We'll do handling for this later
                    }
                    ...
                    final boolean stimulator = inPacket.readByte() > 0;
                    final int numEnchanter = inPacket.readInt();
                    ...
                    if (c.getCharacter().getMeso() < create.getCost()) {
                        return; // H4x
                    }
                    if (c.getCharacter().getInventory(GameConstants.getInventoryType(toCreate)).isFull()) {
                        return; // We'll do handling for this later
                    ...
                    c.getCharacter().gainMeso(-create.getCost(), false);

                    final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                    final Equip toGive = (Equip) ii.getEquipById(toCreate);

                    if (stimulator || numEnchanter > 0) {
                        if (c.getCharacter().haveItem(create.getStimulator(), 1, false, true)) {
                        ...
                        for (int i = 0; i < numEnchanter; i++) {
                            final int enchant = inPacket.readInt();
                            if (c.getCharacter().haveItem(enchant, 1, false, true)) {
                    ...
                    if (!stimulator || Randomizer.nextInt(10) != 0) {
                        ...
                        c.getCharacter().dropMessage(5, "The item was overwhelmed by the stimulator.");
                    }
                    c.sendPacket(EffectPacket.ItemMaker_Success());
            ...
            case 3: { // Making Crystals
                final int etc = inPacket.readInt();
                if (c.getCharacter().haveItem(etc, 100, false, true)) {
            ...
            case 4: { // Disassembling EQ.
                ...
                final byte slot = (byte) inPacket.readInt();

                final Item toUse = c.getCharacter().getInventory(MapleInventoryType.EQUIP).getItem(slot);
    ...
    private static int checkRequiredNRemove(final MapleClient c, final List<Pair<Integer, Integer>> recipe) {
        ...
        for (final Pair<Integer, Integer> p : recipe) {
            if (!c.getCharacter().haveItem(p.getLeft(), p.getRight(), false, true)) {
                return 0;
            }
        }
        for (final Pair<Integer, Integer> p : recipe) {
    ...
    private static boolean hasSkill(final MapleClient c, final int reqlvl) {
        return c.getCharacter().getSkillLevel(SkillFactory.getSkill(PlayerStats.getSkillByJob(1007, c.getCharacter().getJob()))) >= reqlvl;
    }


    public static final void MakeExtractor(final MaplePacketReader inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || !chr.isAlive() || chr.getMap() == null || chr.hasBlockedInventory()) {
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        final int itemId = inPacket.readInt();
        final int fee = inPacket.readInt();
    ...
    public static final void StartHarvest(final MaplePacketReader inPacket, final MapleClient c, final MapleCharacter chr) {
        //its ok if a hacker bypasses this as we do everything in the reactor anyway
        final MapleReactor reactor = c.getCharacter().getMap().getReactorByOid(inPacket.readInt());
        if (reactor == null || !reactor.isAlive() || reactor.getReactorId() > 200011 || chr.getStat().harvestingTool <= 0 || reactor.getTruePosition().distanceSq(chr.getTruePosition()) > 10000 || c.getCharacter().getFatigue() >= 200) {
            return;
        }
        Item item = c.getCharacter().getInventory(MapleInventoryType.EQUIP).getItem((short) c.getCharacter().getStat().harvestingTool);
        if (item == null || ((Equip) item).getDurability() == 0) {
            c.getCharacter().getStat().handleProfessionTool(c.getCharacter());
            return;
        }
        MapleQuestStatus marr = c.getCharacter().getQuestNAdd(MapleQuest.getInstance(GameConstants.HARVEST_TIME));
        if (marr.getCustomData() == null) {
            ...
            c.getCharacter().getMap().broadcastMessage(chr, CField.showHarvesting(chr.getID(), item.getItemId()), false);
        }
    }

    public static final void StopHarvest(final MaplePacketReader inPacket, final MapleClient c, final MapleCharacter chr) {
        //its ok if a hacker bypasses this as we do everything in the reactor anyway
        /*final MapleReactor reactor = c.getPlayer().getMap().getReactorByOid(inPacket.readInt());
         if (reactor == null || !reactor.isAlive() || reactor.getReactorId() > 200011 || chr.getStats().harvestingTool <= 0 || reactor.getTruePosition().distanceSq(chr.getTruePosition()) > 40000.0 || reactor.getState() < 3 || c.getPlayer().getFatigue() >= 100) { //bug in global, so we use this to bug fix
         return;
         }
         Item item = c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((short) c.getPlayer().getStats().harvestingTool);
         if (item == null || ((Equip) item).getDurability() == 0) {
         c.getPlayer().getStats().handleProfessionTool(c.getPlayer());
         return;
         }
    ...
    public static void ProfessionInfo(final MaplePacketReader inPacket, final MapleClient c) { //so pointless
        try {
            String skillid = inPacket.readMapleAsciiString();
            int level1 = inPacket.readInt();
            int level2 = inPacket.readInt();
            int rate;
            if (skillid.startsWith("9200") || skillid.startsWith("9201")) {
                rate = 100;
            } else if (skillid.equals("honorLeveling")) {
                c.sendPacket(CWvsContext.updateSpecialStat(skillid, level1, level2, c.getCharacter().getHonourNextExp()));
                return;
            } else if (skillid.equals("hyper")) {
                c.sendPacket(CWvsContext.updateSpecialStat(skillid, level1, level2, 0));
                return;
            } else {
                rate = Math.max(0, 100 - ((level1 + 1) - c.getCharacter().getProfessionLevel(Integer.parseInt(skillid))) * 20);
            }
            c.sendPacket(CWvsContext.updateSpecialStat(skillid, level1, level2, rate));
        } catch (NumberFormatException nfe) {
        }
    }

    public static final void CraftEffect(final MaplePacketReader inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr.getMapId() != 910001000 && chr.getMap().getExtractorSize() <= 0) {
            return; //ardent mill
        }
        final String effect = inPacket.readMapleAsciiString();
        final Integer profession = craftingEffects.get(effect);
        if (profession != null && (c.getCharacter().getProfessionLevel(profession.intValue()) > 0 || (profession == 92040000 && chr.getMap().getExtractorSize() > 0))) {
            int time = inPacket.readInt();
            if (time > 6000 || time < 3000) {
                time = 4000;
            }
            c.sendPacket(EffectPacket.showOwnCraftingEffect(effect, (byte) chr.getDirection(), time, effect.endsWith("Extract") ? 1 : 0));
    ...
    public static final void CraftMake(final MaplePacketReader inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr.getMapId() != 910001000 && chr.getMap().getExtractorSize() <= 0) {
            return; //ardent mill
        }
        final int something = inPacket.readInt(); //no clue what it is, but its between 288 and 305..
    ...
    public static final void CraftComplete(final MaplePacketReader inPacket, final MapleClient c, final MapleCharacter chr) {
        final int craftID = inPacket.readInt();
        ...
        final int theLevl = c.getCharacter().getProfessionLevel((craftID / 10000) * 10000);
        ...
        if (craftID == 92049000) { //disassembling
            final int extractorId = inPacket.readInt();
            final int itemId = inPacket.readInt();
            final long invId = inPacket.readLong();
            ...
            final int itemId = inPacket.readInt();
            final long invId1 = inPacket.readLong();
            final long invId2 = inPacket.readLong();
            final int reqLevel = ii.getReqLevel(itemId);
    ...
    public static final void UsePot(final MaplePacketReader inPacket, final MapleClient c) {
        final int itemid = inPacket.readInt();
        final Item slot = c.getCharacter().getInventory(MapleInventoryType.USE).getItem(inPacket.readShort());
        ...
        c.sendPacket(CWvsContext.enableActions());
        for (int i = 0; i < c.getCharacter().getImps().length; i++) {
            if (c.getCharacter().getImps()[i] == null) {
                c.getCharacter().getImps()[i] = new MapleImp(itemid);
                c.sendPacket(CWvsContext.updateImp(c.getCharacter().getImps()[i], ImpFlag.SUMMONED.getValue(), i, false));
    ...
    public static final void ClearPot(final MaplePacketReader inPacket, final MapleClient c) {
        final int index = inPacket.readInt() - 1;
        if (index < 0 || index >= c.getCharacter().getImps().length || c.getCharacter().getImps()[index] == null) {
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        c.sendPacket(CWvsContext.updateImp(c.getCharacter().getImps()[index], ImpFlag.REMOVED.getValue(), index, false));
        c.getCharacter().getImps()[index] = null;
    }

    public static final void FeedPot(final MaplePacketReader inPacket, final MapleClient c) {
        final int itemid = inPacket.readInt();
        final Item slot = c.getCharacter().getInventory(GameConstants.getInventoryType(itemid)).getItem((short) inPacket.readInt());
        ...
        if (level <= 0 || level < (Math.min(120, c.getCharacter().getLevel()) - 50) || (GameConstants.getInventoryType(itemid) != MapleInventoryType.ETC && GameConstants.getInventoryType(itemid) != MapleInventoryType.EQUIP)) {
            c.getCharacter().dropMessage(1, "The item must be within 50 levels of you.");
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        final int index = inPacket.readInt() - 1;
        if (index < 0 || index >= c.getCharacter().getImps().length || c.getCharacter().getImps()[index] == null || c.getCharacter().getImps()[index].getLevel() >= (MapleItemInformationProvider.getInstance().getPot(c.getCharacter().getImps()[index].getItemId()).right - 1) || c.getCharacter().getImps()[index].getState() != 1) {
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        ...
        c.getCharacter().getImps()[index].setFullness(c.getCharacter().getImps()[index].getFullness() + (100 * (GameConstants.getInventoryType(itemid) == MapleInventoryType.EQUIP ? 2 : 1)));
        if (Randomizer.nextBoolean()) {
            mask |= ImpFlag.CLOSENESS.getValue();
            c.getCharacter().getImps()[index].setCloseness(c.getCharacter().getImps()[index].getCloseness() + 1 + (Randomizer.nextInt(5 * (GameConstants.getInventoryType(itemid) == MapleInventoryType.EQUIP ? 2 : 1))));
        } else if (Randomizer.nextInt(5) == 0) { //1/10 chance of sickness
            c.getCharacter().getImps()[index].setState(4); //sick
            mask |= ImpFlag.STATE.getValue();
        }
        if (c.getCharacter().getImps()[index].getFullness() >= 1000) {
            c.getCharacter().getImps()[index].setState(1);
            c.getCharacter().getImps()[index].setFullness(0);
            c.getCharacter().getImps()[index].setLevel(c.getCharacter().getImps()[index].getLevel() + 1);
            mask |= ImpFlag.SUMMONED.getValue();
            if (c.getCharacter().getImps()[index].getLevel() >= (MapleItemInformationProvider.getInstance().getPot(c.getCharacter().getImps()[index].getItemId()).right - 1)) {
                c.getCharacter().getImps()[index].setState(5);
            }
        }
        MapleInventoryManipulator.removeFromSlot(c, GameConstants.getInventoryType(itemid), slot.getPosition(), (short) 1, false, false);
        c.sendPacket(CWvsContext.updateImp(c.getCharacter().getImps()[index], mask, index, false));
    }

    public static final void CurePot(final MaplePacketReader inPacket, final MapleClient c) {
        final int itemid = inPacket.readInt();
        final Item slot = c.getCharacter().getInventory(MapleInventoryType.ETC).getItem((short) inPacket.readInt());
        if (slot == null || slot.getQuantity() <= 0 || slot.getItemId() != itemid || itemid / 10000 != 434) {
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        final int index = inPacket.readInt() - 1;
        if (index < 0 || index >= c.getCharacter().getImps().length || c.getCharacter().getImps()[index] == null || c.getCharacter().getImps()[index].getState() != 4) {
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        c.getCharacter().getImps()[index].setState(1);
        c.sendPacket(CWvsContext.updateImp(c.getCharacter().getImps()[index], ImpFlag.STATE.getValue(), index, false));
        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.ETC, slot.getPosition(), (short) 1, false, false);
    }

    public static final void RewardPot(final MaplePacketReader inPacket, final MapleClient c) {
        final int index = inPacket.readInt() - 1;
        if (index < 0 || index >= c.getCharacter().getImps().length || c.getCharacter().getImps()[index] == null || c.getCharacter().getImps()[index].getLevel() < (MapleItemInformationProvider.getInstance().getPot(c.getCharacter().getImps()[index].getItemId()).right - 1)) {
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        final int itemid = GameConstants.getRewardPot(c.getCharacter().getImps()[index].getItemId(), c.getCharacter().getImps()[index].getCloseness());
        if (itemid <= 0 || !MapleInventoryManipulator.checkSpace(c, itemid, (short) 1, "")) {
            c.getCharacter().dropMessage(1, "Please make some space.");
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
        MapleInventoryManipulator.addById(c, itemid, (short) 1, "Item Pot from " + c.getCharacter().getImps()[index].getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
        c.sendPacket(CWvsContext.updateImp(c.getCharacter().getImps()[index], ImpFlag.REMOVED.getValue(), index, false));
        c.getCharacter().getImps()[index] = null;
    }
}
