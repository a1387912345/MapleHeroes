...
public class PackageHandler {
     ...
     */
    public static void handleAction(final MaplePacketReader inPacket, final MapleClient c) {
        final byte operation = inPacket.readByte();

        switch (operation) {
            case 1: { // Start Donald, 13 digit AS
                final String AS13Digit = inPacket.readMapleAsciiString();
                //int unk = inPacket.readInt(); // Theres an int here, value = 1
                //9 = error
                final int conv = c.getCharacter().getConversation();
                if (conv == 2) { // Donald
                    c.sendPacket(CField.sendPackageMSG((byte) 10, loadItems(c.getCharacter())));
                }
                break;
            }
            case 3: { // Send Item
                ...
                final byte inventId = inPacket.readByte();
                final short itemPos = inPacket.readShort();
                final short amount = inPacket.readShort();
                final int mesos = inPacket.readInt();
                final String recipient = inPacket.readMapleAsciiString();
                boolean quickdelivery = inPacket.readByte() > 0;
            ...
            case 5: { // Recieve Package
                ...
                final int packageid = inPacket.readInt();
            ...
            default: {
                System.out.println("Unhandled Package operation : " + inPacket.toString());
                break;
            }
        }
    }
