...
public class FamilyHandler {
    ...
    public static final void RequestFamily(final MaplePacketReader inPacket, MapleClient c) {
        MapleCharacter chr = c.getChannelServer().getPlayerStorage().getCharacterByName(inPacket.readMapleAsciiString());
        if (chr != null) {
            c.sendPacket(FamilyPacket.getFamilyPedigree(chr));
        }
    }

    public static final void OpenFamily(final MaplePacketReader inPacket, MapleClient c) {
        c.sendPacket(FamilyPacket.getFamilyInfo(c.getCharacter()));
    }

    public static final void UseFamily(final MaplePacketReader inPacket, MapleClient c) {
        ...
        switch (entry) {
            case Teleport: //teleport: need add check for if not a safe place
                victim = c.getChannelServer().getPlayerStorage().getCharacterByName(inPacket.readMapleAsciiString());
            ...
            case Summon: // TODO give a check to the player being forced somewhere else..
                ...
                if (FieldLimitType.VipRock.check(c.getCharacter().getMap().getFieldLimit()) || c.getCharacter().isInBlockedMap()) {
                    ...
                    victim.getClient().sendPacket(FamilyPacket.familySummonRequest(c.getCharacter().getName(), c.getCharacter().getMap().getMapName()));
        ...
        if (success) { //again
            ...
            c.sendPacket(FamilyPacket.changeRep(-entry.rep, c.getCharacter().getName()));
    ...
    public static final void FamilyOperation(final MaplePacketReader inPacket, MapleClient c) {
        if (c.getCharacter() == null) {
            return;
        }
        MapleCharacter addChr = c.getChannelServer().getPlayerStorage().getCharacterByName(inPacket.readMapleAsciiString());
        if (addChr == null) {
            ...
            addChr.getClient().sendPacket(FamilyPacket.sendFamilyInvite(c.getCharacter().getID(), c.getCharacter().getLevel(), c.getCharacter().getJob(), c.getCharacter().getName()));
        }
        c.sendPacket(CWvsContext.enableActions());
    }

    public static final void FamilyPrecept(final MaplePacketReader inPacket, MapleClient c) {
        MapleFamily fam = World.Family.getFamily(c.getCharacter().getFamilyId());
        if (fam == null || fam.getLeaderId() != c.getCharacter().getID()) {
            return;
        }
        fam.setNotice(inPacket.readMapleAsciiString());
    }

    public static final void FamilySummon(final MaplePacketReader inPacket, MapleClient c) {
        MapleFamilyBuff cost = MapleFamilyBuff.Summon;
        MapleCharacter tt = c.getChannelServer().getPlayerStorage().getCharacterByName(inPacket.readMapleAsciiString());
    ...
    public static final void DeleteJunior(final MaplePacketReader inPacket, MapleClient c) {
        int juniorid = inPacket.readInt();
    ...
    public static final void DeleteSenior(final MaplePacketReader inPacket, MapleClient c) {
    ...
    public static final void AcceptFamily(MaplePacketReader inPacket, MapleClient c) {
        MapleCharacter inviter = c.getCharacter().getMap().getCharacterById(inPacket.readInt());
        if (inviter != null && c.getCharacter().getSeniorId() == 0 && (c.getCharacter().isGM() || !inviter.isHidden()) && inviter.getLevel() - 20 <= c.getCharacter().getLevel() && inviter.getLevel() >= 10 && inviter.getName().equals(inPacket.readMapleAsciiString()) && inviter.getNoJuniors() < 2 /*&& inviter.getFamily().getGens() < 1000*/ && c.getCharacter().getLevel() >= 10) {
            boolean accepted = inPacket.readByte() > 0;
            inviter.getClient().sendPacket(FamilyPacket.sendFamilyJoinResponse(accepted, c.getCharacter().getName()));
            if (accepted) {
